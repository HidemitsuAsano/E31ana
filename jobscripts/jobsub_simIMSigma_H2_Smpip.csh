...
set CDSDIRSUB="${CDSDIR}simDSTSmpip${DSTVersion}"
...
while ($i < 1600)   
  ...
  set INPFILESP=${DATADIR}"sim_Smpip_0${jobnum}.root"
  
  set CDSFILESP=${CDSDIRSUB}"/simDST_Smpip_0${jobnum}.root"
  
  set OUTFILESP=${OUTDIRSUB}"/simIMsigma_H2_Smpip_0${jobnum}.root"

  echo ${INPFILESP}
  echo ${CDSFILESP} 
  echo ${OUTFILESP}
  
  set lognamesp = "${logdir}/runSmpip$i.log"
...
while (1)
  ...
  if ( $njob < 1 ) then 
    ...
    cd $OUTDIRSUB
    tcsh hadd_sim_h2_Smpip_npi.csh
    tcsh hadd_simhist_h2_Smpip.csh
    cd -
    break
  endif
