...
//        can choose processing single run or run range
//output : 
//1.a text file to give X-T curves of Drift chamebers, except for CDC. 
//2.a root file which contains drift time, drift length and differential of drift lengthdistribution. 
//3.: (optional) a gain map for beam line chambers whose offsets are adjusted wire by wire
                     ...
                     const bool IsDriftTimeCut=true)
{
  ...
  TString outname=Form("xt_run%d_%d.param",run_begin,run_end);
  ...
  for(int ic=0;ic<6;ic++){
    ...
    for(int lay=1;lay<=nlayers;lay++){
      for(int wire=1;wire<=nwires;wire++){
        ...
        //the analyzing layer has only 1 hit.
        //TString hname=Form("d%s_%d_%d_mul1",name,lay,wire);
        
        //all layers have only 1 hit.
        TString hname=Form("d%s_%d_%d_all1hit",name,lay,wire);
        if(cid[ic] == CID_FDC1){
          hname=Form("d%s_%d_%d_BVC1_all1hit",name,lay,wire);
        }
...
void AdjustTDCOffset(ConfMan* conf,DetectorList *dlist,const int run_begin,const int run_end){
  ...
  for(int idc=0;idc<6;idc++){
    ...
    for(int ilr=0;ilr<nlayers;ilr++){
      for(int iwire=0;iwire<nwires;iwire++){
        ...
        double range[2];
        double maxpoint=0.;
        TGraphErrors *gdiff = Differential(*hdt[idc][ilr][iwire],range,maxpoint);
        TString gname = Form("gdiff_%s_%d_%d",name,ilr+1,iwire+1);
        gdiff->SetName(gname);
        gdiff->Write();
        int cr,sl,ch;
        ...
        double pednew=currentped+maxpoint;
        conf->GetGainMapManager()->SetParam(cr,sl,ch, gain_p1,pednew);
        cout << name<< " lay: " << ilr+1<< " wire: " << iwire+1  << " " << maxpoint << endl;
  ...
  if(run_end == 0) outname= Form("GainMapBL_%04d_xt.param",run_begin);  
  ofstream ofsbl(outname.Data());
  cout << "initial parameters are from : " << endl;
  conf->GetGainMapManager()->PrintMapBL(ofsbl);
  return;
}


//input : drift time distribution with TH1F, range 
//output :1. range of drift time distribution
//        2. time when the differential of drift time is maximum.
TGraphErrors* Differential(TH1F &h1,double range[2],double &maximum){
  double y[6000];
  double yerr[6000];
  double x[6000];
