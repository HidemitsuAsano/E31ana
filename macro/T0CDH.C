void tof( TString hname = "tmp.root" )
{
  ...
  TOF_T0_CDH->SetAxisRange(-5,40);
  TOF_T0_CDH->SetStats(0);
  TOF_T0_CDH->Draw();
  double maxy = TOF_T0_CDH->GetMaximum();
  TOF_T0_CDH_E->SetLineColor(7);
  TOF_T0_CDH_E->Draw("same");
  TOF_T0_CDH_Pi->SetLineColor(4);
  TOF_T0_CDH_Pi->Draw("same");
  TOF_T0_CDH_K->SetLineColor(3);
  TOF_T0_CDH_K->Draw("same");
  TOF_T0_CDH_P->SetLineColor(2);
  TOF_T0_CDH_P->Draw("same");
...
void toff2_t0( TString hname = "tmp.root" )
{
  ...
  for( int seg=1; seg<=NumOfT0Segments; seg++ ){
    ...
    double xCDH,yCDH,zCDH, gxCDH,gyCDH,gzCDH;
    double xT0,yT0,zT0, gxT0,gyT0,gzT0;
    conf->GetGeomMapManager()->GetParam( CID_CDH, 19, xCDH, yCDH, zCDH, tmp1, tmp2, tmp3, tmp4, tmp5, tmp6 );
    conf->GetGeomMapManager()->GetGParam( CID_CDH, gxCDH, gyCDH, gzCDH, tmp1, tmp2, tmp3 );
    ...
    xCDH += gxCDH; yCDH += gyCDH; zCDH += gzCDH;
    xT0 += gxT0; yT0 += gyT0; zT0 += gzT0;
    double Length = sqrt( (xCDH-xT0)*(xCDH-xT0) + (yCDH-yT0)*(yCDH-yT0) + (zCDH-zT0)*(zCDH-zT0) );

    h1 = (TH1F*)f->Get( Form("TOF_T0%d_CDH19",seg) );
    c1->cd(seg);
    //h1->Draw();
...
void toff2_cdh( TString hname = "tmp.root" )
{
  double tgatemin1 = -15;
  double tgatemax1 =  15;
  double tgatemin2 = -5;
  ...
  for( int seg=1; seg<=NumOfCDHSegments; seg++ ){
    ...
    h1 = (TH1F*)f->Get( Form("TOF_T03_CDH%d",seg) );
    c1->cd(seg);
    ...
    h2 = (TH1F*)f->Get( Form("ctsubCDH%d",seg) );
    c2->cd(seg);
  ...
  ofstream ofs("tmp.dat");
  conf->GetGainMapManager()->PrintMapHeader(ofs);
  conf->GetGainMapManager()->PrintMap(3,ofs);
  conf->GetGainMapManager()->PrintMap(4,ofs);
  conf->GetGainMapManager()->PrintMap(5,ofs);
  conf->GetGainMapManager()->PrintMap(6,ofs);
  ofs.close();
...
void check_e( TString hname = "tmp.root" )
{
  ...
  for( int seg=1; seg<=NumOfCDHSegments; seg++ ){
    c1->cd(seg);
    gPad->SetLogy();
    h1 = (TH1F*)f->Get( Form("eCDHU%d",seg) );
    h1->Draw();
    h1 = (TH1F*)f->Get( Form("eCDHD%d",seg) );
...
void slew_cdhu( TString hname = "tmp.root" )
{
  double egatemin = 0.5;
  ...
  //slewfunc->SetParameters(0,0);
  for( int seg=1; seg<=NumOfCDHSegments; seg++ ){
    //h1 = (TH1F*)f->Get( Form("etCDHU%d",seg) );
    h1 = (TH1F*)f->Get( Form("dECDH_T03_CDHU%d",seg) );
    if( h1->Integral( h1->GetXaxis()->FindBin(egatemin), h1->GetXaxis()->FindBin(egatemax) )<10 ) continue;
    c1->cd(seg);
    std::cout << "AAA" << slewfunc << std::endl;
    h1->Fit("slewfunc");
    ...
    //double par[2] = { p0,p1 };
    double par[2] = { p0,0 };
    conf->GetSlewingMapManager()->SetParam( CID_CDH, seg, ud, ith, type, npar, par );
  }
  ofstream ofs("tmp.dat");
...
void slew_cdhd( TString hname = "tmp.root" )
{
  double egatemin = 0.5;
  double egatemax = 49.5;
  ...
  for( int seg=1; seg<=NumOfCDHSegments; seg++ ){
    //h1 = (TH1F*)f->Get( Form("etCDHU%d",seg) );
    h1 = (TH1F*)f->Get( Form("dECDH_T03_CDHD%d",seg) );
    if( h1->Integral( h1->GetXaxis()->FindBin(egatemin), h1->GetXaxis()->FindBin(egatemax) )<10 ) continue;
    //if( seg==9 ) continue;
    c1->cd(seg);
    std::cout << "AAA" << slewfunc << std::endl;
...
void toff_cdhu( TString hname = "tmp.root" )
{
  double tgatemin = 500;
...
void toff_cdhd( TString hname = "tmp.root" )
{
  double tgatemin = 500;
  double tgatemax = 2000;
...
void ped_cdhu( TString hname = "tmp.root" )
{
  gSystem->Load("./lib/libAll.so");
  TFile *f = new TFile( hname );
  TH1F *h1,*h2,*h3;
  ...
  for( int seg=1; seg<=NumOfCDHSegments; seg++ ){
    ...
    h2 = (TH1F*)f->Get( Form("AwTCDHU%d",seg) );
    h3 = new TH1F(*h1);
    h3->Add(h1,h2,1,-1);
    c1->cd(seg);
...
void ped_cdhd( TString hname = "tmp.root" )
{
  gSystem->Load("./lib/libAll.so");
  TFile *f = new TFile( hname );
  TH1F *h1,*h2,*h3;
  ...
  for( int seg=1; seg<=NumOfCDHSegments; seg++ ){
    ...
    h2 = (TH1F*)f->Get( Form("AwTCDHD%d",seg) );
    h3 = new TH1F(*h1);
    h3->Add(h1,h2,1,-1);
    c1->cd(seg);
    h2->Fit("landau","q","",0,1500);
    h3->Fit("gaus","q","",50,100);
    gPad->SetLogy();
    h1->SetAxisRange(0,1500,"X");
    h1->Draw();
    h2->SetLineColor(2); h2->Draw("same");
