...
int main( int argc, char **argv )
{
  ...
  for( int ev=0; ev<tree->GetEntries(); ev++ ){
	...
	if( fabs(in_mom.Mag()-out_mom)<10e-5 ) break;
      ...
      beam.SetD5mom(D5mom); // Set D5 momentum w/ mom resolution w/ 0.3%
	...
	if( anaInfo->minDCA() && fcInfo.pid()==F_Proton ){
	  CDSInfo *dca=anaInfo->minDCA();
	  HodoscopeLikeHit *cdh=dca->CDH(cdsMan);
	  ...
	  if( !FChit_mc ) continue;
	  Track *p_track =MyMCTools::track(mcData, FChit_mc->trackID());
       
	  TVector3 vtx=dca->vertexBeam();
	  std::cout<<"CDH hit PDG : "<<cdh_mc->pdg()<<std::endl;
	  std::cout<<"FC  hit PDG : "<<FChit_mc->pdg()<<std::endl;
	  if( FChit_mc->pdg()==2212 ){
	    ...
	    std::cout<<"FL by data : "<<10.*fcInfo.fl()<<std::endl;
