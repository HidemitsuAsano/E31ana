...
class EventAnalysisReadCDCeff : public EventTemp
{
public:
  EventAnalysisReadCDCeff();
  ~EventAnalysisReadCDCeff(){};
private:
  int t0, t1;
...
EventAnalysisReadCDCeff::EventAnalysisReadCDCeff() : EventTemp()
{
}

bool EventAnalysisReadCDCeff::UAna(TKOHitCollection *tko)
{
  Event_Number++;
...
void EventAnalysisReadCDCeff::Initialize(ConfMan *conf)
{
  confMan = conf;
...
bool EventAnalysisReadCDCeff::cdsFileMatching()
{
...
bool EventAnalysisReadCDCeff::anaFileMatching()
{
...
bool EventAnalysisReadCDCeff::Clear(bool flag)
{
  header-> Clear();
  blMan-> Clear();
  cdsMan-> Clear();
...
void EventAnalysisReadCDCeff::USca(int nsca, unsigned int *sca)
{
  Block_Event_Number++;
...
void EventAnalysisReadCDCeff::Finalize()
{
  delete blMan;
  delete cdsMan;
  delete cdsHeader;
