...
int MyFractionFitter::Fit()
{
  ...
  double plist[1]={0.5};
  fFitter->ExecuteCommand("SET ERRDEF",plist,1);
  // Make sure the correct likelihood computation is used
  fFitter->SetObjectFit(this);
  fFitter-> ExecuteCommand("MINIMIZE",0,0);

  // int status = fractionFitter->ExecuteCommand("MINIMIZE",0,0);
  // cout<<"> MyFractionFitter::Fit() status : "<<status<<endl;
  return -1;
}

MyFractionFitter::MyFractionFitter(TH1 *h1, TObjArray *mc) : fFitter(0), fData(0), fPlot(0)
{
  ...
  fFitter = TVirtualFitter::Fitter(this, fMC.size());
  fFitter->Clear();
  fFitter->SetObjectFit(this);
  fFitter->SetFCN(fractionFCN);
  ...
  for( int i=0; i<(int)fMC.size(); i++ ){
    ...
    fFitter->SetParameter(i, (TString)fMC[i]->GetName(), frac, 0.01*frac, 0.0, 2.0*frac);
  }
