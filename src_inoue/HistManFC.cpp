...
void HistManBeamAna::initFC()
{
  ...
  new TH2F("NC_overbeta_dE",       "NC 1/#beta vs dE", 500, -0.5, 4.5, 500, 0.0, 25.);
  new TH2F("NC_overbeta_dE_wtar",  "NC 1/#beta vs dE", 500, -0.5, 4.5, 500, 0.0, 25.);
  ...
  new TH2F("KNpip_KNpim_MM_wK0",  "d(K^{-}, n #pi^{+}) vs d(K^{-}, n #pi^{-})", 200, 1.0, 2.0,200, 1.0, 2.0);
  ...
  new TH2F("KNpip_KNpim_MM_wK0_wtar",  "d(K^{-}, n #pi^{+}) vs d(K^{-}, n #pi^{-})", 200, 1.0, 2.0,200, 1.0, 2.0);
  ...
  for( int seg=1; seg<=112; seg++ ){
    ...
    new TH1F(Form("T0NC%d_offset_9MeVeewtar", seg), Form("NC seg%d offset by T0-NC", seg), 20000, -100, 100);
    ...
    new TH2F(Form("euctm_NC_%d", seg), Form("NC seg%d up dE vs time", seg),   500, 0, 25, 500, -10, 10);
    new TH2F(Form("edctm_NC_%d", seg), Form("NC seg%d down dE vs time", seg), 500, 0, 25, 500, -10, 10);
  ...
  if( fBeamPID==Beam_Pion || fBeamPID==Beam_Kaon ){
	...
	if( fCVC_hit.size()==0 ){
	  ...
	  for( int lay=0; lay<8; lay++ ){
		...
		if( fNC_hit[lay][i]->emean()>8 ) h1 = getTH("NC_overbeta_8MeVee"), h1-> Fill(1./beta);
		h2 = getTH2("NC_overbeta_dE"), h2-> Fill(1./beta, fNC_hit[lay][i]->emean());
		if( MyTools::IsTarget(vtx) ){
		  h1 = getTH("NC_overbeta_wtar"), h1-> Fill(1./beta);
		  if( fNC_hit[lay][i]->emean()>8 ) h1 = getTH("NC_overbeta_8MeVee_wtar"), h1-> Fill(1./beta);
		  ...
		  if( fBeamPID==Beam_Kaon && header-> IsTrig(Trig_Neutral) ){
			...
			if( TrackTools::Calc2HelixVertex(cdsTrack1, cdsTrack2, vtxh1, vtxh2) ){
			  ...
			  if( cdsTrack1->GetMomentum(vtxh1, pim_mom, true, true) ){
				...
				h2 = getTH2("KNpip_KNpim_MM_wK0"), h2-> Fill((beam_lmom+TargetLmom-fn_lmom-pip_lmom).M(), 
							          	     (beam_lmom+TargetLmom-fn_lmom-pim_lmom).M());
			      }
			      else{
				...
				if( K0_MIN<im && im<K0_MAX ){
				  ...
				  h2 = getTH2("KNpip_KNpim_MM_wK0_wtar"), h2-> Fill((beam_lmom+TargetLmom-fn_lmom-pip_lmom).M(), 
										    (beam_lmom+TargetLmom-fn_lmom-pim_lmom).M());
				}
				else{
