...
HistManwMC::HistManwMC(TFile *f, ConfMan *conf)
  : rtFile(f), confMan(conf), simReader(0), blMan(0), cdsMan(0), bltrackMan(0), cdstrackMan(0)
{
  Clear();
}
...
bool HistManwMC::ana(EventHeader *header)
{
  ...
  if( good_vtx && fCVC_hit.size()==0 && fBVC_hit.size()==0 && nNC()>0 ){
    std::cout<<" NC ana start"<<std::endl;
    if( !mcData ){
	...
	if( !mcData ) nc_pos.SetY(hit_1st->hitpos());
	...
	for( int lay=0; lay<8; lay++ ){
	  for( int i=0; i<fNC_hit[lay].size(); i++ ){
	    if( !NC_test_flag[j] && 2*j<fNC_hit[lay][i]->emean() ){
	      ...
	      if( !mcData ) nc_pos.SetY(fNC_hit[lay][i]->hitpos());
...
void HistManwMC::fill(EventHeader *header)
{
  ...
  for( int i=0; i<cdstrackMan->nGoodTrack(); i++ ){
    ...
    int pid = track-> PID();
    if( pid!=CDS_PiMinus && pid!=CDS_PiPlus && pid!=CDS_Proton && pid!=CDS_Deuteron && pid!=CDS_Kaon ) continue;

    TVector3 vtx_b, vtx_cds;
    if( !track-> GetVertex(fT0pos, fTrackBPC->GetMomDir(), vtx_b, vtx_cds) ) continue;
    TVector3 p;
    if( !track-> GetMomentum(vtx_cds, p, true, true) ) continue;
    TLorentzVector lmom;
    lmom.SetVectM(p, cdsMass[pid]);

    double beam_out, beam_tof;
    ELossTools::CalcElossBeamTGeo(fT0pos, vtx_b, fD5mom, parMass[fBeamPID], beam_out, beam_tof);
    TVector3 beam_mom = fTrackBPC->GetMomDir();
    beam_mom.SetMag(beam_out);
    TLorentzVector beam_lmom;
    beam_lmom.SetVectM(beam_mom, parMass[fBeamPID]);
    TLorentzVector missing_lmom = beam_lmom+TGT_LMOM-lmom;
    if( fBeamPID==Beam_Kaon && GeomTools::GetID(vtx_b)==CID_Fiducial ){
      if( pid==CDS_PiMinus ){
	h1 = (TH1F*)rtFile-> Get("KCDSpim_MM"), h1-> Fill(missing_lmom.M());
      }
      if( pid==CDS_PiPlus ){
	h1 = (TH1F*)rtFile-> Get("KCDSpip_MM"), h1-> Fill(missing_lmom.M());
      }
      if( pid==CDS_Kaon ){
	h1 = (TH1F*)rtFile-> Get("KCDSkm_MM"), h1-> Fill(missing_lmom.M());
      }
      if( pid==CDS_Proton ){
	h1 = (TH1F*)rtFile-> Get("KCDSp_MM"), h1-> Fill(missing_lmom.M());
      }
      if( pid==CDS_Deuteron ){
	h1 = (TH1F*)rtFile-> Get("KCDSd_MM"), h1-> Fill(missing_lmom.M());
      }
    }
  }
	...
	if( fTrackPim[i]-> GetMomentum(vtx_pim, pim_mom, true, true) &&
	  ...
	  if( GeomTools::GetID(vtxb)==CID_Fiducial ){
	    if( fBeamPID==Beam_Kaon ){
	      ...
	      h1 = (TH1F*)rtFile-> Get("KCDSpipi_MM"), h1-> Fill(mm_kn_pipi);
	      h2 = (TH2F*)rtFile-> Get("CDS_IM_pipi_KCDSpipi_MM"), h2-> Fill(im_pipi, mm_kn_pipi);
	      h1 = (TH1F*)rtFile-> Get("KCDSpim_MM_pip"), h1-> Fill(mm_kn_pim);
	      h1 = (TH1F*)rtFile-> Get("KCDSpip_MM_pim"), h1-> Fill(mm_kn_pip);
	      h2 = (TH2F*)rtFile-> Get("KCDSpim_KCDSpip_MM"), h2-> Fill(mm_kn_pim, mm_kn_pip);
	    }
	  }
	}
      }
  ...
  if( fFPID==F_Neutron && trigNC_flag ){
	...
	if( fBeamPID==Beam_Kaon ){
	  ...
	  if( fiducial_flag ){
	    ...
	    if( fKNpipi_N_flag ) h2 = (TH2F*)rtFile-> Get("Npim_Npip_IM_wN"), h2-> Fill(im_npim, im_npip);
	    if( fKNpipi_K0_flag ) h2 = (TH2F*)rtFile-> Get("Npim_Npip_IM_wK0"), h2-> Fill(im_npim, im_npip);
		...
		if( mm_kn<1.43 ) h2 = (TH2F*)rtFile-> Get("KNpim_KNpip_MM_woAll_wN_143"), h2-> Fill(mm_knpim, mm_knpip);
		if( mm_kn<1.44 ) h2 = (TH2F*)rtFile-> Get("KNpim_KNpip_MM_woAll_wN_144"), h2-> Fill(mm_knpim, mm_knpip);
	  ...
	  if( mcData ){
		...
		if( fKNpipi_N_flag ){
		  ...
		  if( fKNpim_Sp_flag ){
		    if( fDecayMode==-1     )  h1 = (TH1F*)rtFile-> Get("L1405_pipi_hit_woAll_wN_wSp_Sm"), h1-> Fill(fYstarLmom.M());
		    else if( fDecayMode==1 )  h1 = (TH1F*)rtFile-> Get("L1405_pipi_hit_woAll_wN_wSp_Sp"), h1-> Fill(fYstarLmom.M());
		    if( !fKNpip_Sm_flag ){
		      if( fDecayMode==-1     )  h1 = (TH1F*)rtFile-> Get("L1405_pipi_hit_woAll_wN_wSp_wo_Sm_Sm"), h1-> Fill(fYstarLmom.M());
		      else if( fDecayMode==1 )  h1 = (TH1F*)rtFile-> Get("L1405_pipi_hit_woAll_wN_wSp_wo_Sm_Sp"), h1-> Fill(fYstarLmom.M());
		    }
		  }

		  if( fKNpip_Sm_flag ){
		    if( fDecayMode==-1     )  h1 = (TH1F*)rtFile-> Get("L1405_pipi_hit_woAll_wN_wSm_Sm"), h1-> Fill(fYstarLmom.M());
		    else if( fDecayMode==1 )  h1 = (TH1F*)rtFile-> Get("L1405_pipi_hit_woAll_wN_wSm_Sp"), h1-> Fill(fYstarLmom.M());
		    if( fKNpim_Sp_flag ){
		      if( fDecayMode==-1     )  h1 = (TH1F*)rtFile-> Get("L1405_pipi_hit_woAll_wN_wSm_woSp_Sm"), h1-> Fill(fYstarLmom.M());
		      else if( fDecayMode==1 )  h1 = (TH1F*)rtFile-> Get("L1405_pipi_hit_woAll_wN_wSm_woSp_Sp"), h1-> Fill(fYstarLmom.M());
		    }
		  }
		}
	      }
