...
private:
  ...
  void HistSemiLocal();
  void FillLinear( const int &ic,LocalTrack *track);
...
bool EventAnalysisMomentum::UAna( TKOHitCollection *tko )
{
...
#endif
#if TRACK
  trackMan->DoTracking( blMan, confMan, true );
#endif
  rtFile->cd();
  if(trackMan->ntrackBLC1()==1&&trackMan->ntrackBLC2()==1){
  ...
  for(int i=0;i<2;i++){
    int ntr=trackMan->ntrackBLDC(tmpid[i]);
...
void EventAnalysisMomentum::FillLinear(const int &c,LocalTrack* track)
{
  TH1F* h1;
  TH2F* h2;
  double a1,b1,c1,d1;
