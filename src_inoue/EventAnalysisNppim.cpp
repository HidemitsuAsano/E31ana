...
class EventAnalysisNppim: public EventTemp
{
public:
  EventAnalysisNppim();
  ~EventAnalysisNppim();
...
EventAnalysisNppim::EventAnalysisNppim()
  : EventTemp()
{
  std::cout<<"EventAnalysisNppim::Constractor Call"<<std::endl;
}
...
void EventAnalysisNppim::Initialize( ConfMan *conf )
{
  std::cout<<"EventAnalysisNppim::Initialization START"<<std::endl;
  ...
  std::cout<<"EventAnalysisNppim::Initialization FINISH"<<std::endl;
}

void EventAnalysisNppim::USca( int nsca, unsigned int *sca )
{
  Block_Event_Number++;
...
bool EventAnalysisNppim::UAna( TKOHitCollection *tko )
{
  TH1F *h1;
  TH2F *h2;
  ...
  if( header->IsTrig(Trig_Neutral) ){
    ...
    if( NC_beta<0.9 && NCdE>8.){
      if( trackPim.size()==1 && trackP.size()==1 ){
	TVector3 vtxPim, vtxP;
	if( !TrackTools::Calc2HelixVertex(trackPim[0], trackP[0], vtxPim, vtxP) ){
	...
	if( !trackP[0]->GetMomentum(vtxP, p_mom, true, true) ){
	  std::cout<<"  !!! pi+ track::GetMomentum false !!!"<<std::endl;
	  Clear();
	  return true;
	}

	TVector3 vtx_mean = 0.5*(vtxPim+vtxP);
...
void EventAnalysisNppim::Finalize()
{
  std::cout<<"EventAnalysisNppim Finish   Event Number : "<<Event_Number<<"  Event Number on Spill : "<<Event_Number_onSpill<<std::endl;
  gFile->Write();
  confMan-> SaveParams();
  confMan-> SaveCode();
  gFile->Close();
...
{
  EventAnalysisNppim *event = new EventAnalysisNppim();
