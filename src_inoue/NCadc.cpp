...
int main(int argc, char** argv)
{
  ...
  for( int seg=1; seg<=112; seg++ ){
    f-> cd();
    vector<double> adc_param=CalibADC(conf, of, CID_NC, seg, 10.0);
    xxx[index]=seg;
  ...
  gra_pede_u-> Write("gra_NC_pede_up");
  gra_pede_d-> Write("gra_NC_pede_down");
  gra_gain_u-> Write("gra_NC_gain_up");
  gra_gain_d-> Write("gra_NC_gain_down");
  gra_thre_u-> Write("gra_NC_thre_up");
  gra_thre_d-> Write("gra_NC_thre_down");
