...
class EventAnalysisCalib3 : public EventTemp
{
 public:
  EventAnalysisCalib3();
  ~EventAnalysisCalib3(){};
...
EventAnalysisCalib3::EventAnalysisCalib3() : EventTemp()
{
}

void EventAnalysisCalib3::Initialize(ConfMan *conf)
{
  confMan = conf;
...
void EventAnalysisCalib3::USca(int nsca, unsigned int *sca)
{
  Block_Event_Number++;
...
bool EventAnalysisCalib3::UAna(TKOHitCollection *tko)
{
  Event_Number++;
  ...
  if( anaInfo->beam(0)->flag() ){
    fillBHDT0offset(anaInfo->beam(), blMan);
  }
...
void EventAnalysisCalib3::Finalize()
{
  gFile-> cd();
  gFile-> Write();
  gFile-> Close();
...
bool EventAnalysisCalib3::Clear(bool flag)
{
  //  cdsTreeNum=-1;
