...
void PrintAll(TString filename="tmp.root", double amin=0,double amax=1000)
{
  PrintCDS(filename,"cds.ps",amin,amax);
  PrintBeamline(filename,"beam.ps",amin,amax);
  PrintForwardTOF(filename,"ftof.ps",amin,amax);
}

void DrawRawHist(){
  //  PrintAll("/s/e17/hashimoto/k18ana/run45/evanaraw0.root"); 
  PrintAll("test.root"); 
}
