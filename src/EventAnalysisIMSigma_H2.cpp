//H. Asano
// to analyze k-p->pi+Sigma-, pi-Sigma+ events RUN62
//----------------------------------------------------------------//
//----------------------------------------------------------------//
...
private:
  ...
  TVector3 vtx_pi_cdc; // 
  TVector3 CA_pi;//Closest Approach Point of CDS pip-pim tracks
  TVector3 CDH_Pos;//neutron candidate
  TVector3 CDH_Pos_pi;//pi CDC track projected position at CDH
...
void EventAnalysis::Initialize( ConfMan *conf )
{
  ...
  npiTree->Branch( "kfSpmode_mom_beam",   &kfSpmode_mom_beam );
  npiTree->Branch( "kfSpmode_mom_pip", &kfSpmode_mom_pip );
  npiTree->Branch( "kfSpmode_mom_pim", &kfSpmode_mom_pim );
  npiTree->Branch( "kfSpmode_mom_n", &kfSpmode_mom_n );
  npiTree->Branch( "kfSpmode_chi2", &kfSpmode_chi2 );
  npiTree->Branch( "kfSpmode_NDF", &kfSpmode_NDF );
  npiTree->Branch( "kfSpmode_status", &kfSpmode_status );
  npiTree->Branch( "kfSpmode_pvalue", &kfSpmode_pvalue );
  npiTree->Branch( "kfSmmode_mom_beam",   &kfSmmode_mom_beam );
  npiTree->Branch( "kfSmmode_mom_pip", &kfSmmode_mom_pip );
  npiTree->Branch( "kfSmmode_mom_pim", &kfSmmode_mom_pim );
  npiTree->Branch( "kfSmmode_mom_n", &kfSmmode_mom_n );
  npiTree->Branch( "kfSmmode_chi2", &kfSmmode_chi2 );
  npiTree->Branch( "kfSmmode_NDF", &kfSmmode_NDF );
  npiTree->Branch( "kfSmmode_status", &kfSmmode_status );
  npiTree->Branch( "kfSmmode_pvalue", &kfSmmode_pvalue );
  npiTree->Branch( "kf_flag", &kf_flag );
...
//**--**--**--**--**--**--**--**--**--**--**--**--**--**//
{
  ...
  *trackMan = *trackMan_CDC;
  ...
  //K x CDH2 trigger
  bool IsTrigKCDH2 = header->IsTrig(Trig_KCDH2);
  if(IsTrigKCDH2){
    Tools::Fill1D(Form("Trigger"),1);
  }else{
    //Clear(nAbort_KCDH2trg);
    //return true;
  }
  ...
  //PID beam
  //4th argument :1 -> H2 parameter for beam 
  const int beamPID = Util::BeamPID(header, ctmT0, blMan,1);
  ...
  const int nIDedTrack = Util::CDSChargedAna(
    ...
    LVec_beam, ctmT0,vCDHseg,pim_ID,pip_ID,km_ID,p_ID,pim_cdhprojected,pip_cdhprojected,false,1);
  ...
  if( pimpipFlag &&
    ...
    for( int it=0; it<trackMan->nGoodTrack(); it++ ) {
      ...
      const int pid = TrackTools::PIDcorr3(mom,mass2);
      track->SetPID( pid );
      Tools::Fill2D( "PID_CDS_beta_select", 1./beta_calc, mom );
      Tools::Fill2D( "PID_CDS_select", mass2, mom );
      if(pid == CDS_PiMinus || pid == CDS_PiPlus){
        //Tools::Fill2D("PID_CDS_PI_beta_select",1./beta_calc,mom);
        //Tools::Fill2D("PID_CDS_PI_select",mass2,mom);
        tofpi = tof;//tree val.
      }
    ...
    */
    ...
    CDSTrack *track_pi;
    if(pip_ID.size()==1){
      track_pi = trackMan->Track( pip_ID.at(0) ); // should be only 1 track
      chargepi=1;
    }
    if(pim_ID.size()==1){
      track_pi = trackMan->Track( pim_ID.at(0) ); // should be only 1 track
      chargepi=0;
    }
    ...
    const double dcapivtx =  (vtx_pi-vtx_beam_wpi).Mag();
    const TVector3 vtxpi_mean = 0.5*(vtx_pi+vtx_beam_wpi);
    ...
    */
    ...
    const TVector3 P_missn = (LVec_target+LVec_beam-LVec_pi-LVec_n).Vect();
    LVec_nmiss.SetVectM( P_missn, piMass );
    ...
    Tools::Fill2D(Form("Vtx_ZX_nofid"),vtxpi_mean.Z(),vtxpi_mean.X());
    Tools::Fill2D(Form("Vtx_ZY_nofid"),vtxpi_mean.Z(),vtxpi_mean.Y());
    Tools::Fill2D(Form("Vtx_XY_nofid"),vtxpi_mean.X(),vtxpi_mean.Y());
    //Fiducial cuts OK
    if( (!IsVtxDoubleCheck && (GeomTools::GetID(vtx_react)==CID_Fiducial)) ){ 
      ...
      Tools::Fill2D(Form("Vtx_ZX_fid"),vtxpi_mean.Z(),vtxpi_mean.X());
      Tools::Fill2D(Form("Vtx_ZY_fid"),vtxpi_mean.Z(),vtxpi_mean.Y());
      Tools::Fill2D(Form("Vtx_XY_fid"),vtxpi_mean.X(),vtxpi_mean.Y());
      ...
      if( NBetaOK && NdEOK ) {
        ...
        */
        ...
        mom_pi = LVec_pi;        // 4-momentum(pi+ or pi-)
        ...
        if(chargepi==1) CDH_Pos_pi = pip_cdhprojected;
        else if(chargepi==0) CDH_Pos_pi = pim_cdhprojected;
