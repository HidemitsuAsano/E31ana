...
static const Double_t LowLimit[5] = { -50, -2*TMath::Pi(), -0.5,-100,-10 };
static const Double_t  UpLimit[5] = { 50, 2*TMath::Pi(), 0.5,100,10 };
  ...
  while(dist<128)
    {
      ...
      double dlen_b=MathTools::dfunc_LTH(a,dlineu,phi_b,par);
      double dlen_a=MathTools::dfunc_LTH(a,dlineu,phi_a,par);
  ...
  for(int i=0;i<trial;i++)
    {
      ...
      double dlen=MathTools::dfunc_LTH(a,dlineu,phi,par);
      if(dlen*dlen_b<=0 ) {phi=(phi_b+phi)/2.0;dist=dist/2.0;}
      else  {phi=(phi_a+phi)/2.0;dist=dist/2.0;}      
    }
  //
  // helix nearest point
  TVector3 hnest=MathTools::GetPosition(phi,par);
...
static void fcn2( Int_t &npar, Double_t *gin, Double_t &f, Double_t *par, Int_t iflag )
{
  ...
  for( Int_t i=0; i<gNumOfHits; i++ ){
	...
	MathTools::PointToCircle(par, hitx, hity, fitx, fity,dist);
	chisq +=pow( ( dist - gDrift[i] )/gWeight[i] , 2 );
      }
    else
      {
	if(!LineToHelix(gWirePos[i], gWireDir[i],par, dist, fittmp) ) 
	  //	if(!MathTools::PointToHelix(gHitPos[i],fittmp,par) ) 
	  {	
...
{  
  ...
  for( Int_t i=0; i<NumOfHits; i++ ){    
	...
	chisq +=pow((dist-Drift[i])/Weight[i], 2);
      ...
      {
	MathTools::LineToHelix( WirePos[i], WireDir[i], Par, fittmp, fittmp2, dist);
	chisq += pow( (dist-Drift[i])/Weight[i] , 2 );
