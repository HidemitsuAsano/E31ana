...
//**--**--**--**--**--**--**--**--**--**--**--**--**--**//
{
  ...
  *trackMan = *trackMan_CDC; 
  ...
  double ctmBHD;
  int pid_beam = -1; // 0:pi 1:K 3:else
  for( int i=0; i<blMan->nBHD(); i++ ){
  ...
  if( pid_beam== -1  ){ //** unidentified particle is discarded (other than pi/K) **//
  ...
  for( int i=0; i<bltrackMan->ntrackBLC2(); i++ ){
    ...
    if( blc2->CheckRange(blcuts::blc2_time_window_min, blcuts::blc2_time_window_max) ){
	  ...
	  bltrackMan->trackBLC2(i)->chi2all()<blc2_chi2_max ) blc2id = i;
    }	
  }
  ...
  TLorentzVector L3_target;  // 4-Momentum(deuteron-target) in LAB
  ...
  TLorentzVector L3_targetCM;  // 4-Momentum(deuteron-target) in CM
  ...
  for( int it=0; it<trackMan->nGoodTrack(); it++ ){
    ...
    //** calculation of NeutralBetaCDH and squared-mass **//
