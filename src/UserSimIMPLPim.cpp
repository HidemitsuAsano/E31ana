...
int main( int argc, char** argv )
{
  ...
  for(int ip=0;ip<kin::npart;ip++){
    for(int im=0;im<16;im++){
      std::cout << kin::covValpLpim[ip][im] << " ";
  ...
  for( int ip=0; ip<kin::npart; ip++ ){
    ...
    for( int j=0; j<4; j++ ){
      for( int k=0; k<4; k++ ){
	if( j==k ){
	  (*covParticleSpmode[ip])[j][k] = kin::covValpLpim[ip][n]; // only diagonal elements
  ...
  ppimpimTree->Branch( "kfSmmode_mom_beam",   &kfMomBeamSmmode );
  ppimpimTree->Branch( "kfSmmode_mom_pip", &kfMom_pip_Smmode );
  ppimpimTree->Branch( "kfSmmode_mom_pim", &kfMom_pim_Smmode );
  ppimpimTree->Branch( "kfSmmode_mom_n", &kfMom_n_Smmode );
  ppimpimTree->Branch( "kfSmmode_chi2", &kf_chi2_Smmode );
  ppimpimTree->Branch( "kfSmmode_NDF", &kf_NDF_Smmode );
  ppimpimTree->Branch( "kfSmmode_status", &kf_status_Smmode );
  ppimpimTree->Branch( "kfSmmode_pvalue", &kf_pvalue_Smmode );
  ...
  for( int iev=0; iev<exen; iev++ ){
    ...
    const int PDG_pLpim[kin::npart] = {321, -211, 3122, 2212, 2212,  -211}; // pi-Sigma+
          ...
          if( NBetaOK && NdEOK ){
            ...
            for( int ip=0; ip<kin::npart; ip++ ){
              ParticleSpmode[ip] = TFitParticlePxPyPz(str_particleSpmode[ip], str_particleSpmode[ip], &TV_measSpmode[ip],
                  pdg->GetParticle(PDG_pLpim[ip])->Mass(), covParticleSpmode[ip]);
            ...
            TFitConstraintM ConstMSSpmode = TFitConstraintM("M_Sp", "M_Sp", 0, 0, pdg->GetParticle(PDG_pLpim[kin::Sp])->Mass());
