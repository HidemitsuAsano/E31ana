...
do
evnum=$(printf %04d $i)
out=$(printf root/ana/Run65/cdctrackingwslew/run65_%04d_v%02d.root $i $vr)
...
bsub -ql -o tmp.log ./bin/cdctrackingwslew "$confe" "$evnum" "$out" "$in" "$track" "$all"
done
...
track=$(printf root/ana/Run65/cdctracking/run65_%04d_v%02d.root $i $trackvr)
all=$(printf root/ana/Run65/myall/run65_%04d_v%02d.root $i $anavr)
bsub -ql -o tmp.log ./bin/cdctrackingwslew "$conf" "$evnum" "$out" "$in" "$track" "$all"
done
