...
#do
#evnum=$(printf %04d $i)
#out=$(printf root/ana/Run68/myall/run68_%04d_v%02d.root $i $vr)
...
##bsub -qs -o tmp.log ./bin/myall "$conf" "$evnum" "$out" "$in"
#bsub -qs -o tmp.log ./bin/myall "$conf" "$evnum" "$out" "$in" "$track"
#done
