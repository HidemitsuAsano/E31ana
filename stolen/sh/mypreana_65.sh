...
#bsub -ql -o tmp.log ./bin/mypreana "$confe" "$evnum" "$out" "$in"
bsub -ql -o tmp.log ./bin/mypreana "$confe" "$evnum" "$out" "$in" "$track"
done
...
track=$(printf root/ana/Run65/cdctracking/run65_%04d_v%02d.root $i $trackvr)
#track=$(printf /group/had/knucl/e15/noumi/root/ana/Run65/cdctracking/run65_%04d_v%02d.root $i $trackvr)
#bsub -ql -o tmp.log ./bin/mypreana "$conf" "$evnum" "$out" "$in"
bsub -ql -o tmp.log ./bin/mypreana "$conf" "$evnum" "$out" "$in" "$track"
done
