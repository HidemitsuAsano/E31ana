...
evnum=$(printf %04d $i)
out=$(printf root/ana/Run64/cdctracking/run64_%04d_v%02d_kill%02d.root $i $vr $killed)
in=$(printf data/Run64/run64_%04d.dat $i)
bin=$(printf ./bin/cdctracking_killedl%d $killed)
bsub -qs -o tmp.log "$bin" "$conf" "$evnum" "$out" "$in" 
done
done
