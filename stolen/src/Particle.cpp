...
double pBeam::CalcVertexTime(const TVector3 &vertex)
{
  ...
  if(!SIM){
    ...
    ELossTools::CalcElossBeamTGeo(vertex,t0pos(),mom(),mass(),momout,tof);
  }
  return t0time()+tof;
}
...
void pPC::CalcMomBVC(const  TVector3 &vertex, int pid, bool tmp){
...
void pPC::CalcMom(pBeam *beam,const  TVector3 &vertex, int pid, bool tmp){
  double beamtime=beam->CalcVertexTime(vertex); 
  //  TVector3 fdc1  
  TVector3 pdir=(fdc1pos()-vertex).Unit();
  TVector3 uswkpos=fdc1pos()+(250-fdc1pos().Z())/pdir.Z()*pdir;
