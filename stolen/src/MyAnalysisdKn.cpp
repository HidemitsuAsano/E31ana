...
MyAnalysisdKn::MyAnalysisdKn(TFile* rt, ConfMan* conf)
{
  Initialize(conf);
  CutCondition();
  Clear();
...
void MyAnalysisdKn::Clear()
{
}

bool MyAnalysisdKn::DoAnalysis(ConfMan* conf, EventHeader* header, BeamLineHitMan* blMan, BeamLineTrackMan* bltrackMan, CDSHitMan* cdsMan, CDSTrackingMan* cdstrackMan, Particle* particle)
{
  rtFile->cd();
  ...
  if(particle->nPiplus()>0){
    for(int f=1; f<2; f++){
      ...
      FillHist(Form("dKn_pip_MMvsMomentum_%s",frame[f].Data()),nc_mmass[f],nc_mmom[f]);
      FillHist(Form("dKn_pip_MMvsCosT_%s",frame[f].Data()),nc_mmass[f],nc_mcost[f]);
    }
  }
  if(particle->nPiminus()>0){
    for(int f=1; f<2; f++){
      ...
      FillHist(Form("dKn_pim_MMvsMomentum_%s",frame[f].Data()),nc_mmass[f],nc_mmom[f]);
      FillHist(Form("dKn_pim_MMvsCosT_%s",frame[f].Data()),nc_mmass[f],nc_mcost[f]);
    }
  }
  if(particle->nKaon()>0){
    for(int f=1; f<2; f++){
      ...
      FillHist(Form("dKn_k_MMvsMomentum_%s",frame[f].Data()),nc_mmass[f],nc_mmom[f]);
      FillHist(Form("dKn_k_MMvsCosT_%s",frame[f].Data()),nc_mmass[f],nc_mcost[f]);
    }
  }
  if(particle->nProton()>0){
    for(int f=1; f<2; f++){
      ...
      FillHist(Form("dKn_p_MMvsMomentum_%s",frame[f].Data()),nc_mmass[f],nc_mmom[f]);
      FillHist(Form("dKn_p_MMvsCosT_%s",frame[f].Data()),nc_mmass[f],nc_mcost[f]);
    }
  }
  if(particle->nPiplus()==1&&particle->nPiminus()==1){
    for(int f=1; f<2; f++){
      ...
      FillHist(Form("dKn_pipi_MMvsMomentum_%s",frame[f].Data()),nc_mmass[f],nc_mmom[f]);
      FillHist(Form("dKn_pipi_MMvsCosT_%s",frame[f].Data()),nc_mmass[f],nc_mcost[f]);
    }
  }
  if(k0flag){
    for(int f=1; f<2; f++){
      ...
      FillHist(Form("dKn_k0_MMvsMomentum_%s",frame[f].Data()),nc_mmass[f],nc_mmom[f]);
      FillHist(Form("dKn_k0_MMvsCosT_%s",frame[f].Data()),nc_mmass[f],nc_mcost[f]);
    }
  }
  if(spflag){
    for(int f=1; f<2; f++){
      ...
      FillHist(Form("dKn_sp_MMvsMomentum_%s",frame[f].Data()),nc_mmass[f],nc_mmom[f]);
      FillHist(Form("dKn_sp_MMvsCosT_%s",frame[f].Data()),nc_mmass[f],nc_mcost[f]);
      ...
      FillHist(Form("dKn_s_MMvsMomentum_%s",frame[f].Data()),nc_mmass[f],nc_mmom[f]);
      FillHist(Form("dKn_s_MMvsCosT_%s",frame[f].Data()),nc_mmass[f],nc_mcost[f]);
      if(!k0flag){
        FillHist(Form("dKn_sp_wok0_MMvsMomentum_%s",frame[f].Data()),nc_mmass[f],nc_mmom[f]);
        FillHist(Form("dKn_sp_wok0_MMvsCosT_%s",frame[f].Data()),nc_mmass[f],nc_mcost[f]);
        FillHist(Form("dKn_s_wok0_MMvsMomentum_%s",frame[f].Data()),nc_mmass[f],nc_mmom[f]);
        FillHist(Form("dKn_s_wok0_MMvsCosT_%s",frame[f].Data()),nc_mmass[f],nc_mcost[f]);
      }
    }
  }
  if(smflag){
    for(int f=1; f<2; f++){
      ...
      FillHist(Form("dKn_sm_MMvsMomentum_%s",frame[f].Data()),nc_mmass[f],nc_mmom[f]);
      FillHist(Form("dKn_sm_MMvsCosT_%s",frame[f].Data()),nc_mmass[f],nc_mcost[f]);
      FillHist(Form("n_s_CosTvsMomentum_%s",frame[f].Data()),nc_cost[f],nc_mom[f]);
      FillHist(Form("n_s_CosTvsPhi_%s",frame[f].Data()),nc_cost[f],nc_phi[f]);
      FillHist(Form("dKn_s_MMvsMomentum_%s",frame[f].Data()),nc_mmass[f],nc_mmom[f]);
      FillHist(Form("dKn_s_MMvsCosT_%s",frame[f].Data()),nc_mmass[f],nc_mcost[f]);
      if(!k0flag){
        FillHist(Form("dKn_sm_wok0_MMvsMomentum_%s",frame[f].Data()),nc_mmass[f],nc_mmom[f]);
        FillHist(Form("dKn_sm_wok0_MMvsCosT_%s",frame[f].Data()),nc_mmass[f],nc_mcost[f]);
        FillHist(Form("dKn_s_wok0_MMvsMomentum_%s",frame[f].Data()),nc_mmass[f],nc_mmom[f]);
        FillHist(Form("dKn_s_wok0_MMvsCosT_%s",frame[f].Data()),nc_mmass[f],nc_mcost[f]);
      }
    }
  ...
  for(int f=1; f<2; f++){
    ...
    FillHist(Form("dKn%s_MMvsMomentum_%s",pname[cds->pid()].Data(),frame[f].Data()),ncds_mmass[f],ncds_mmom[f]);
    FillHist(Form("dKn%s_MMvsCosT_%s",pname[cds->pid()].Data(),frame[f].Data()),ncds_mmass[f],ncds_mcost[f]);
  }
...
bool MyAnalysisdKn::FillHist(TString name, double val1, int weight)
{
...
bool MyAnalysisdKn::FillHist(TString name, TString val1, int weight)
{
  TH1F* h1 = (TH1F*)gFile -> Get(name);
  if(h1&&weight>0){
    for(int i=0; i<weight; i++){
...
bool MyAnalysisdKn::FillHist(TString name, double val1, double val2, int weight)
{
...
bool MyAnalysisdKn::FillHist(TString name, TString val1, TString val2, int weight)
{
  TH2F* h2 = (TH2F*)gFile -> Get(name);
  if(h2&&weight>0){
    for(int i=0; i<weight; i++){
...
void MyAnalysisdKn::CutCondition()
{
  /* K0 mass cut */ 
  double mean  = 0.4976;
  double sigma = 0.0069;
...
bool MyAnalysisdKn::Initialize(ConfMan* confMan)
{
  std::cout << "### MyAnalysisdKn::Initialize ###" << std::endl;

  std::string ofname = confMan->GetOutFileName();
  ofname.insert(ofname.find(".root"),"_anadKn");
  ...
  // Missing mass
  std::cout << "Define Histograms for d(K-,n)X M.M." << std::endl;
  new TH2F("dKn_MMvsMomentum_CM", "d(K^{-},n)X missing mass vs. missing momentum;MM_{d}(n) (GeV/c^{2});Missing momentum (GeV/c)", 2000, 0.0, 2.0, 1000, 0.0, 1.0);
  new TH2F("dKn_MMvsCosT_CM", "d(K^{-},n)X missing mass vs. cos#theta;MM_{d}(n) (GeV/c^{2});cos#theta", 2000, 0.0, 2.0, 2000, -1.0, 1.0);
  for(int ip=0; ip<8; ip++){
    new TH2F(Form("dKn_%s_MMvsMomentum_CM",pname[ip].Data()), "d(K^{-},n)X missing mass vs. missing momentum;MM_{d}(n) (GeV/c^{2});Missing momentum (GeV/c)", 2000, 0.0, 2.0, 1000, 0.0, 1.0);
    new TH2F(Form("dKn_%s_MMvsCosT_CM",pname[ip].Data()), "d(K^{-},n)X missing mass vs. cos#theta;MM_{d}(n) (GeV/c^{2});cos#theta", 2000, 0.0, 2.0, 2000, -1.0, 1.0);
    new TH2F(Form("dKn%s_MMvsMomentum_CM",pname[ip].Data()), Form("d(K^{-},n%s)X missing mass vs. missing momentum;MM_{d}(n%s) (GeV/c^{2});Missing momentum (GeV/c)",pname2[ip].Data(),pname2[ip].Data()), 2000, 0.0, 2.0, 1000, 0.0, 1.0);
    new TH2F(Form("dKn%s_MMvsCosT_CM",pname[ip].Data()), Form("d(K^{-},n%s)X missing mass vs. cos#theta;MM_{d}(n%s) (GeV/c^{2});cos#theta",pname2[ip].Data(),pname2[ip].Data()), 2000, 0.0, 2.0, 2000, -1.0, 1.0);
  }
  for(int ip=0; ip<6; ip++){
    new TH2F(Form("dKn_%s_MMvsMomentum_CM",proname[ip].Data()), "d(K^{-},n)X missing mass vs. missing momentum;MM_{d}(n) (GeV/c^{2});Missing momentum (GeV/c)", 2000, 0.0, 2.0, 1000, 0.0, 1.0);
    new TH2F(Form("dKn_%s_MMvsCosT_CM",proname[ip].Data()), "d(K^{-},n)X missing mass vs. cos#theta;MM_{d}(n) (GeV/c^{2});cos#theta", 2000, 0.0, 2.0, 2000, -1.0, 1.0);
    if(2<=ip&&ip<=4){
      new TH2F(Form("dKn_%s_wok0_MMvsMomentum_CM",proname[ip].Data()), "d(K^{-},n)X missing mass vs. missing momentum;MM_{d}(n) (GeV/c^{2});Missing momentum (GeV/c)", 2000, 0.0, 2.0, 1000, 0.0, 1.0);
      new TH2F(Form("dKn_%s_wok0_MMvsCosT_CM",proname[ip].Data()), "d(K^{-},n)X missing mass vs. cos#theta;MM_{d}(n) (GeV/c^{2});cos#theta", 2000, 0.0, 2.0, 2000, -1.0, 1.0);
    }
    new TH2F(Form("dKn%s_MMvsMomentum_CM",proname[ip].Data()), Form("d(K^{-},n%s)X missing mass vs. missing momentum;MM_{d}(n%s) (GeV/c^{2});Missing momentum (GeV/c)",proname2[ip].Data(),proname2[ip].Data()), 2000, 0.0, 2.0, 1000, 0.0, 1.0);
    new TH2F(Form("dKn%s_MMvsCosT_CM",proname[ip].Data()), Form("d(K^{-},n%s)X missing mass vs. cos#theta;MM_{d}(n%s) (GeV/c^{2});cos#theta",proname2[ip].Data(),proname2[ip].Data()), 2000, 0.0, 2.0, 2000, -1.0, 1.0);
  }
