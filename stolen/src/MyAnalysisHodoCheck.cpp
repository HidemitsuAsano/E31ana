// MyAnalysisHodoCheck.cpp

#include "MyAnalysisHodoCheck.h"

MyAnalysisHodoCheck::MyAnalysisHodoCheck(TFile* rt, ConfMan* conf)
{
	Initialize(conf);
	CutCondition();
	Clear();
...
MyAnalysisHodoCheck::~MyAnalysisHodoCheck()
{
	Clear();
	rtFile->cd();
	rtFile->Write();
...
void MyAnalysisHodoCheck::Clear()
{
}

bool MyAnalysisHodoCheck::DoAnalysis(ConfMan* conf, EventHeader* header, BeamLineHitMan* blMan, BeamLineTrackMan* bltrackMan)
{
  ...
  const int nhodo = 13;
  int hodoid[nhodo]={
    CID_BHD,CID_T0,CID_DEF,CID_BPD,CID_BVC,CID_CVC,
    CID_PC,CID_NC,CID_BD,CID_LB,CID_WVC,CID_HVC1,CID_HVC2
  };
...
bool MyAnalysisHodoCheck::FillHist(TString name, double val1, int weight)
{
...
bool MyAnalysisHodoCheck::FillHist(TString name, TString val1, int weight)
{
  TH1F* h1 = (TH1F*)gFile -> Get(name);
  if(h1&&weight>0){
    for(int i=0; i<weight; i++){
...
bool MyAnalysisHodoCheck::FillHist(TString name, double val1, double val2, int weight)
{
...
bool MyAnalysisHodoCheck::FillHist(TString name, TString val1, TString val2, int weight)
{
  TH2F* h2 = (TH2F*)gFile -> Get(name);
  if(h2&&weight>0){
    for(int i=0; i<weight; i++){
...
void MyAnalysisHodoCheck::CutCondition()
{
}

double MyAnalysisHodoCheck::CalcTimeOffs(HodoscopeLikeHit* hit, Particle* particle)
{
  return 0;
}

double MyAnalysisHodoCheck::CalcTimeSubOffs(HodoscopeLikeHit* hit, Particle* particle)
{
  return 0;
}

bool MyAnalysisHodoCheck::Initialize(ConfMan* confMan)
{
  std::cout << "### MyAnalysisHodoCheck::Initialize ###" << std::endl;

  confFile = confMan;
  DetectorList *dlist=DetectorList::GetInstance();

  std::string ofname = confMan->GetOutFileName();
  ofname.insert(ofname.find(".root"),"_anaHodoCheck");
