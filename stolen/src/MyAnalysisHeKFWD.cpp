// MyAnalysisHeKFWD.cpp

#include "MyAnalysisHeKFWD.h"

MyAnalysisHeKFWD::MyAnalysisHeKFWD(TFile* rt, ConfMan* conf)
{
  Initialize(conf);
  CutCondition();
  Clear();
...
MyAnalysisHeKFWD::~MyAnalysisHeKFWD()
{
  Clear();
  rtFile->cd();
  rtFile->Write();
...
void MyAnalysisHeKFWD::Clear()
{
}

bool MyAnalysisHeKFWD::DoAnalysis(ConfMan* conf, EventHeader* header, BeamLineHitMan* blMan, BeamLineTrackMan* bltrackMan, CDSHitMan* cdsMan, CDSTrackingMan* cdstrackMan, Particle* particle)
{
  rtFile->cd();
  ...
  if(particle->nPC()>0) charged = true;
...
bool MyAnalysisHeKFWD::DoNeutralAna(Particle* particle, pBeam* beam, pCDS* cds, TVector3 vertex)
{
  // ############### //
  // NC hit decision //
  // ############### //
...
bool MyAnalysisHeKFWD::DoChargedAna(Particle* particle, pBeam* beam, pCDS* cds, TVector3 vertex)
{
  // ##################### //
...
bool MyAnalysisHeKFWD::FillHist(TString name, double val1, int weight)
{
...
bool MyAnalysisHeKFWD::FillHist(TString name, TString val1, int weight)
{
  TH1F* h1 = (TH1F*)gFile -> Get(name);
  if(h1&&weight>0){
    for(int i=0; i<weight; i++){
...
bool MyAnalysisHeKFWD::FillHist(TString name, double val1, double val2, int weight)
{
...
bool MyAnalysisHeKFWD::FillHist(TString name, TString val1, TString val2, int weight)
{
  TH2F* h2 = (TH2F*)gFile -> Get(name);
  if(h2&&weight>0){
    for(int i=0; i<weight; i++){
...
void MyAnalysisHeKFWD::CutCondition()
{
  /* K0 mass cut */ 
  double mean  = 0.4976;
  double sigma = 0.0069;
...
bool MyAnalysisHeKFWD::Initialize(ConfMan* confMan)
{
  std::cout << "### MyAnalysisHeKFWD::Initialize ###" << std::endl;

  std::string ofname = confMan->GetOutFileName();
  ofname.insert(ofname.find(".root"),"_anaHeKFWD");
