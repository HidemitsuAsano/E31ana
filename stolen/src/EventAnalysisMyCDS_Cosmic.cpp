...
bool EventAnalysisMyCDC::UAna( TKOHitCollection *tko )
{
  ...
  { int status = confMan->CheckEvNum( Event_Number, Block_Event_Number );
    ...
    // Selection //
    //if(MulCDH!=2 || nTrack<1 || !header->IsTrig(Trig_Cosmic)){
    if(MulCDH!=2 || nTrack<1){
      header->Clear();
      blMan->Clear();
      cdsMan->Clear();
      cdstrackMan->Clear();
    ...
    for(int it=0; it<cdstrackMan->nGoodTrack(); it++){
      ...
      // CDC
      if(Chi<30){
        h1 = (TH1F*)gFile->Get( "CDC_Efficiency" ); h1->Fill( 0 );
      }
      {
        ...
        for( int layer=1; layer<=NumOfCDCLayers; layer++ ){
          ...
          if(nhit[layer-1]>=1&&Chi<30){
            h1 = (TH1F*)gFile->Get( "CDC_Efficiency" ); h1->Fill( layer );
          }
