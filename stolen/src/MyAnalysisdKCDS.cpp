...
MyAnalysisdKCDS::MyAnalysisdKCDS(TFile* rt, ConfMan* conf)
{
  Initialize(conf);
  CutCondition();
  Clear();
...
MyAnalysisdKCDS::~MyAnalysisdKCDS()
{
  Clear();
  rtFile->cd();
  rtFile->Write();
...
void MyAnalysisdKCDS::Clear()
{
}

bool MyAnalysisdKCDS::DoAnalysis(ConfMan* conf, EventHeader* header, BeamLineHitMan* blMan, BeamLineTrackMan* bltrackMan, CDSHitMan* cdsMan, CDSTrackingMan* cdstrackMan, Particle* particle)
{
  rtFile->cd();
  ...
  if(cdstrackMan->nGoodTrack()==2&&particle->nCDS()==2) neutral = true;
  if(cdstrackMan->nGoodTrack()==3&&particle->nCDS()==3) charged = true;
  if(neutral&&charged) return true;
  FillHist("EventNumber",2); /* Both Neutral and Charged */
  if(neutral&&(particle->nPiplus()!=1||particle->nPiminus()!=1)) return true;
  ...
  if(neutral) DoNeutralAna(particle,beam,cds,vertex,cdsMan);
...
bool MyAnalysisdKCDS::DoNeutralAna(Particle* particle, pBeam* beam, pCDS* cds, TVector3 vertex, CDSHitMan* cdsMan)
{
  DetectorList *dlist=DetectorList::GetInstance();
  ...
  if(pim->ncdh()!=1||pip->ncdh()!=1) return true;
  int seg_pim = pim->cdhseg(0);
  int seg_pip = pip->cdhseg(0);
  ...
  for(int i=0; i<cdsMan->nCDH(); i++){
    ...
    if(hit->CheckRange()){
      int tmpseg = hit->seg();
      if(tmpseg==seg_pim) continue;
      if(tmpseg==seg_pip) continue;
      if(hit->emean()<4.0) continue;
      cdh = hit;
    }
  }
  if(cdh==0) return true;
  double beam_tof = beam->CalcVertexTime(vertex);
  double tof = cdh->ctmean();
  TVector3 hitpos;
  hitpos.SetXYZ(cdh->pos().X(),cdh->pos().Y(),cdh->hitpos());
  double cdh_angle = TMath::ATan(hitpos.Y()/hitpos.X());
  if(hitpos.Y()<0&&hitpos.X()<0) cdh_angle-=TMath::Pi();
  if(hitpos.Y()<0&&hitpos.X()>0) cdh_angle+=TMath::Pi();
  //std::cout << "neutral hit? >> (" << hitpos.X() << ", " << hitpos.Y() << ")" 
  //  << "; " << cdh_angle <<  std::endl;
  for(int layer=14;layer<=15;layer++){
    int mult = cdsMan->nCDC(layer);
    for(int i=0;i<mult;i++){
      CDCHit *hit = cdsMan->CDC(layer,i);
      int wire = hit->wire();
      double wire_x = hit->wxp();
      double wire_y = hit->wyp();
      double cdc_angle = TMath::ATan(wire_y/wire_x);
      if(wire_y<0&&wire_x<0) cdc_angle-=TMath::Pi();
      if(wire_y<0&&wire_x>0) cdc_angle+=TMath::Pi();
      double diff = TMath::Abs(cdc_angle-cdh_angle);
      if(diff>TMath::Pi()) diff=TMath::Abs(diff-TMath::Pi()*2.0);
      FillHist("CDSN_AngleDifference",diff*180/TMath::Pi());
      if(diff<TMath::Pi()/12){
        //std::cout << "CDC-l" << layer << "-w" << wire << " >> (" << wire_x << ", " << wire_y << ")" 
        //  << "; " << cdc_angle << std::endl;
        return true;
      }
      FillHist("CDSN_AngleDifference_Selected",diff*180/TMath::Pi());
    }
  }

  double fl = (vertex - hitpos).Mag();
  double beta = fl/tof/(Const*100.0);
  double momn = nMass/sqrt(1/beta/beta-1);
  TVector3 mom = (vertex-hitpos).Unit()*momn;
  FillHist("CDSN_Overbeta",1/beta);
  FillHist("CDSN_OverbetavsEnergy",1/beta,cdh->emean());
  FillHist("CDSN_FlightLength",fl);
  FillHist("CDSN_TOF",tof);
  FillHist("CDSN_Momentum",momn);

  pCDS* nc = new pCDS();
  nc->SetPID(CDS_Neutron);
  nc->SetMomentum(momn);
  nc->SetMass(nMass);
  nc->SetMass2(nMass*nMass);
  nc->SetBeta(beta);
  nc->SetTOF(tof);
  nc->SetFL(fl);
  nc->SetVertexBeam(vertex);
  nc->SetMomDir(mom);
...
bool MyAnalysisdKCDS::DoChargedAna(Particle* particle, pBeam* beam, pCDS* cds, TVector3 vertex)
{
  DetectorList *dlist=DetectorList::GetInstance();
...
bool MyAnalysisdKCDS::FillHist(TString name, double val1, int weight)
{
...
bool MyAnalysisdKCDS::FillHist(TString name, TString val1, int weight)
{
  TH1F* h1 = (TH1F*)gFile -> Get(name);
  if(h1&&weight>0){
    for(int i=0; i<weight; i++){
...
bool MyAnalysisdKCDS::FillHist(TString name, double val1, double val2, int weight)
{
...
bool MyAnalysisdKCDS::FillHist(TString name, TString val1, TString val2, int weight)
{
  TH2F* h2 = (TH2F*)gFile -> Get(name);
  if(h2&&weight>0){
    for(int i=0; i<weight; i++){
...
void MyAnalysisdKCDS::CutCondition()
{
  /* K0 mass cut */ 
  double mean  = 0.49732;
...
bool MyAnalysisdKCDS::Initialize(ConfMan* confMan)
{
  std::cout << "### MyAnalysisdKCDS::Initialize ###" << std::endl;

  std::string ofname = confMan->GetOutFileName();
  ofname.insert(ofname.find(".root"),"_anadKCDS");
  ...
  new TH1F( "CDSN_Overbeta", "1/#beta;1/#beta;Counts", 8000, 0, 8);
  new TH1F( "CDSN_Overbeta_withdECut", "1/#beta;1/#beta;Counts", 5000, 0, 5);
  new TH1F( "CDSN_Overbeta_Selected", "1/#beta;1/#beta;Counts", 5000, 0, 5);
  new TH2F( "CDSN_OverbetavsEnergy", "1/#beta vs. Energy deposit;1/#beta;Energy deposit (MeVee)", 1000, -0, 10, 5000, 0, 100);
  new TH2F( "CDSN_OverbetavsEnergy_withdECut", "1/#beta vs. Energy deposit;1/#beta;Energy deposit (MeVee)", 1000, -0, 10, 5000, 0, 100);
  new TH1F( "CDSN_FlightLength", "Flight length;Flight length (cm);Counts", 3000, 0, 300);
  new TH1F( "CDSN_TOF", "Time of flight;TOF (ns);Counts", 5000, 0, 50);
  new TH1F( "CDSN_Momentum", "Momentum;Momentum (GeV/c);Counts", 2000, 0, 2);
  new TH1F( "CDSN_AngleDifference", "Angle difference;Angle diff. (deg.);Counts", 3600, 0, 360);
  new TH1F( "CDSN_AngleDifference_Selected", "Angle difference;Angle diff. (deg.);Counts", 3600, 0, 360);
  // pi+
