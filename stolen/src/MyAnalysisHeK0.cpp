...
#include "MyAnalysisHeK0.h"

MyAnalysisHeK0::MyAnalysisHeK0(TFile* rt, ConfMan* conf)
{
  Initialize(conf);
  CutCondition();
  Clear();
...
void MyAnalysisHeK0::Clear()
{
}

bool MyAnalysisHeK0::DoAnalysis(ConfMan* conf, EventHeader* header, BeamLineHitMan* blMan, BeamLineTrackMan* bltrackMan, CDSHitMan* cdsMan, CDSTrackingMan* cdstrackMan, Particle* particle)
{
...
bool MyAnalysisHeK0::FillHist(TString name, double val1, int weight)
{
...
bool MyAnalysisHeK0::FillHist(TString name, TString val1, int weight)
{
  TH1F* h1 = (TH1F*)gFile -> Get(name);
  if(h1&&weight>0){
    for(int i=0; i<weight; i++){
...
bool MyAnalysisHeK0::FillHist(TString name, double val1, double val2, int weight)
{
...
bool MyAnalysisHeK0::FillHist(TString name, TString val1, TString val2, int weight)
{
  TH2F* h2 = (TH2F*)gFile -> Get(name);
  if(h2&&weight>0){
    for(int i=0; i<weight; i++){
...
void MyAnalysisHeK0::CutCondition()
{
  /* K0 mass cut */ 
  double mean  = 0.4976;
  double sigma = 0.0069;
...
bool MyAnalysisHeK0::Initialize(ConfMan* confMan)
{
  std::cout << "### MyAnalysisHeK0::Initialize ###" << std::endl;
