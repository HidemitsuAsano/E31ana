...
#ifndef TimeOffsetMapMan_h
#define TimeOffsetMapMan_h 1
...
class TimeOffsetMap : public TObject
{
 public:
  TimeOffsetMap();
  ~TimeOffsetMap() {};
 ...
 public:
  ...
  ClassDef(TimeOffsetMap,1);
};

class TimeOffsetMapMan : public TObject
{
 public:
  TimeOffsetMapMan();
  ~TimeOffsetMapMan();
  ...
  TimeOffsetMapMan( const TimeOffsetMapMan &right );
 private:
  ...
  typedef std::map <unsigned int, TimeOffsetMap> TimeOffsetMapContainer;
  TimeOffsetMapContainer slewingContainer;

 public:
  void SetTimeOffsetMapMan( const TimeOffsetMapContainer container )  { slewingContainer = container; }
  ...
  bool GetParam( const int &cid, const int &seg, const int &run, const int &ith, int &type,
		 int &npar, std::vector <double> &par );
  bool GetParam( const int &cid, const int &seg, const int &run, const int &ith, int &type,
		 int &npar, double *par );
  bool SetParam( const int &cid, const int &seg, const int &run, const int &ith, const int &type,
		 const int &npar, double *par );
  bool SetParam( const int &cid, const int &seg, const int &run, const int &ith, const int &type,
		 const int &npar, const std::vector <double> &par );
  double CalcCValue( const int &cid, const int &seg, const int &run,
		     const double &time, const double &de );

  double CalcDATValue( const int &cid, const int &seg, const int &run,
