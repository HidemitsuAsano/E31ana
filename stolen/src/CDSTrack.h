...
class CDSTrack : public TObject
{
 ...
 public:
  ...
  bool GetCDHHit2( CDSHitMan *cdsMan,int &seg, double &time,double &phidiff);
  ...
  void SetPhi( const double &aphi ) { phi = aphi; }
  ...
  bool Retiming(CDSHitMan *cdsMan,ConfMan *conf,double mass,bool SLEW=false,int layer=-1,int wire=-1);
  ...
  bool HelixFitting(CDSHitMan *cdsMan,int layer=-1,int wire=-1);
