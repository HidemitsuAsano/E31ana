...
// + -- + -- + -- + -- + -- + -- + -- + -- + -- + -- + -- + -- + -- + -- + -- + -- + -- + //
void SlewingMapMan::SetFileNameCDC( const std::string & filename )
{
...
bool SlewingMapMan::Initialize()
{
  ...
  if( FileNameCDC!=DefaultFileName ){
    if( (fp=fopen(FileNameCDC.c_str(), "r"))==0 ){
      std::cerr << " File open fail. [" << FileNameCDC << "]" << std::endl;
      exit(-1);
    }
    ...
    fclose(fp);
    //    std::cout << "[" << funcname << "] CDC is initialized." << std::endl;
    std::cout <<" CDC.";
  }
				  ...
				  const double &time, const double &de )
{
  ...
  while( (is=(slewingContainer.find(key))) != slewingContainer.end() ){
	...
	//ctime = ctime + p0/sqrt(de-p1);
	ctime = ctime + p0/sqrt(de) + p1 + p2*de + p3/de;
	//ctime = ctime + p0/sqrt(de) + p1 + p2/de;
	ith++;
	key = KEY(cid,seg,ud,ith);
	break;
...
// + -- + -- + -- + -- + -- + -- + -- + -- + -- + -- + -- + -- + -- + -- + -- + -- + -- + //
void SlewingMapMan::PrintMapCDC( std::ostream &p_out )
{
  static const std::string funcname = "SlewingMapMan::PrintMapCDC";
  PrintMapHeader(p_out);
