...
bool MyAnalysisBL::DoAnalysis(ConfMan* conf, EventHeader* header, BeamLineHitMan* blMan, BeamLineTrackMan* bltrackMan, Particle* particle, double _mom)
{
	...
	for(int i=0; i<bltrackMan->ntrackBLC1(); i++){
		...
		if(track->nhit(0)>4&&track->nhit(1)>4){
	...
	for(int i=0; i<bltrackMan->ntrackBLC2(); i++){
		...
		if(track->nhit(0)>4&&track->nhit(1)>4){
			if(maxcid==CID_BLC2){BLDCParam->SetLayerDeadBLC2a(maxlayer-1,true);}
			if(maxcid==CID_BLC2){BLDCParam->SetLayerDeadBLC2b(maxlayer-1,true);}
		}
	}
#ifdef DEBUG
	std::cout << "------------------" << std::endl;
	...
	for(int i=0; i<bltrackMan->ntrackBPC(); i++){
		...
		if(track->nhit(0)>3&&track->nhit(1)>3){BLDCParam->SetLayerDeadBPC(maxlayer-1,true);}
	}
#ifdef DEBUG
	std::cout << "------------------" << std::endl;
#endif
...

	{
		...
		for(idc=0;idc<2;idc++){
			...
			for(int i=ntrack_first[idc]; i<bltrackMan->ntrackBLDC(dccid[idc]); i++){
