...
bool EventAnalysisMyCDC::UAna( TKOHitCollection *tko )
{
	...
	// Selection //
	if(MulT0!=1 || nTrack<1 || header->IsTrig(Trig_Cosmic)){
		header->Clear();
		blMan->Clear();
		cdsMan->Clear();
		cdstrackMan->Clear();
	...
	for(int it=0; it<cdstrackMan->nGoodTrack(); it++){
		...
		if(nIH!=1||nCDH!=1) continue;
		// CDC
		int killedlayer = cdstrackMan->GetKilledLayer();
		h1 = (TH1F*)gFile->Get( Form("CDC_Efficiency") ); 
		h1 -> Fill(0);
		{
			...
			for( int i=0; i<cdsMan->nCDC(layer); i++ ){
				...
				double par[5];
				TVector3 tmpvtx;
				track->GetParameters(CID_CDC,par,tmpvtx);
				TVector3 lpos = hit->gpos();
				TVector3 ldir = hit->gdir();
				TVector3 vtx1,vtx2;
				double tmpdis;
				if(!MathTools::LineToHelix(lpos,ldir,par,vtx1,vtx2,tmpdis)) continue;
				h1 = (TH1F*)gFile->Get( Form("CDC_Layer%d_Distance",layer) ); h1->Fill( tmpdis );
				h2 = (TH2F*)gFile->Get( Form("CDC_Layer%d_ChiSquarevDistance",layer) ); h2->Fill( Chi,tmpdis );
				if(dis>tmpdis) dis = tmpdis;
			}
			h1 = (TH1F*)gFile->Get( Form("CDC_Layer%d_Distance_Closed",layer) ); h1->Fill( dis );
			h2 = (TH2F*)gFile->Get( Form("CDC_Layer%d_ChiSquarevDistance_Closed",layer) ); h2->Fill( Chi,dis );
			if(dis<9){
				h1 = (TH1F*)gFile->Get( Form("CDC_Efficiency") ); 
				h1 -> Fill(layer);
			}
...
void EventAnalysisMyCDC::InitializeHistogram()
{
	...
	for(int idc=0;idc<ndc;idc++){
		...
		new TH1F( Form("%s_ChiSquare",dcname[idc].Data()), Form("ChiSquare %s;#Chi^{2}/ndf;Counts",dcname[idc].Data()), 400, 0, 100 );
		...
		for( int layer=1; layer<=NumOfLayers[idc]; layer++ ){
			...
			new TH1F( Form("%s_Layer%d_Distance",dcname[idc].Data(),layer), Form("Distance between Hit position and Track %s Layer%d;Distance (mm);Counts",dcname[idc].Data(),layer), 500, 0.0, 50.0 );
			new TH1F( Form("%s_Layer%d_Distance_Closed",dcname[idc].Data(),layer), Form("Distance between Hit position and Track %s Layer%d;Distance (mm);Counts",dcname[idc].Data(),layer), 500, 0.0, 50.0 );
			new TH2F( Form("%s_Layer%d_ChiSquarevDistance",dcname[idc].Data(),layer), Form("ChiSquare vs. Distance %s Layer%d;#Chi^{2}/ndf;Distance (mm)",dcname[idc].Data(),layer), 400, 0, 100, 500, 0.0, 50.0 );
			new TH2F( Form("%s_Layer%d_ChiSquarevDistance_Closed",dcname[idc].Data(),layer), Form("ChiSquare vs. Distance %s Layer%d;#Chi^{2}/ndf;Distance (mm)",dcname[idc].Data(),layer), 400, 0, 100, 500, 0.0, 50.0 );
		}
	}
