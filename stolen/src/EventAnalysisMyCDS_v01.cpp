...
bool EventAnalysisMyCDC::UAna( TKOHitCollection *tko )
{
  ...
  { int status = confMan->CheckEvNum( Event_Number, Block_Event_Number );
    ...
    // Selection //
    if(MulT0!=1 || nGoodTrack!=2 || header->IsTrig(Trig_Cosmic)){
      header->Clear();
      blMan->Clear();
      cdsMan->Clear();
      trackMan->Clear();
    ...
    for(int it=0; it<trackMan->nGoodTrack(); it++){
      ...
      double T0_Timing = 0.0;
      double CDH_Timing[36] = {0.0};
      ...
      for(int i=0; i<nCDH; i++){
        ...
        h1 = (TH1F*)gFile->Get("T0CDH_TOF"); h1->Fill(TOF);
...
void EventAnalysisMyCDC::InitializeHistogram()
{
  ...
  // TOF
  new TH1F("T0CDH_TOF","TOf",1000,0.,100.);

  new TH1F( "Mass","Mass (GeV/c2)", 200, 0.0, 1.0 );
  new TH1F( "Momentum","Momentum (GeV/c)", 300, -1.5, 1.5 );
  new TH2F("TOFvMomentum","TOF T0-CDH vs. Momentum;Time of flight (ns);Momentum (GeV/c)",500,0,50,300,-1.5,1.5);
  new TH2F("dE_IHvMomentum","IH dE vs. Momentum;dE (MeV);Momentum (GeV/c)",500,0,5,300,-1.5,1.5);
  new TH2F("dE_CDHvMomentum","CDH dE vs. Momentum;dE (MeV);Momentum (GeV/c)",500,0,5,300,-1.5,1.5);
  ...
  for(int idc=0;idc<ndc;idc++){
    ...
    for( int layer=1; layer<=NumOfLayers[idc]; layer++ ){
      ...
      new TH2F( Form("%s_Layer%d_TimevResidual",dcname[idc].Data(),layer), Form("Drift time vs. Drift length %s Layer%d;Drift time (ns);Residual (mm)",dcname[idc].Data(),layer), 400, -500, 1500, 100, -0.5, 0.5 );
    }
  }
