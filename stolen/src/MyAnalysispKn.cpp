...
MyAnalysispKn::MyAnalysispKn(TFile* rt, ConfMan* conf)
{
  Initialize(conf);
  CutCondition();
  Clear();
...
void MyAnalysispKn::Clear()
{
}

bool MyAnalysispKn::DoAnalysis(ConfMan* conf, EventHeader* header, BeamLineHitMan* blMan, BeamLineTrackMan* bltrackMan, CDSHitMan* cdsMan, CDSTrackingMan* cdstrackMan, Particle* particle)
{
  rtFile->cd();
  ...
  if(particle->nPiplus()>0){
    for(int f=1; f<2; f++){
      ...
      FillHist(Form("pKn_pip_MMvsMomentum_%s",frame[f].Data()),nc_mmass[f],nc_mmom[f]);
      FillHist(Form("pKn_pip_MMvsCosT_%s",frame[f].Data()),nc_mmass[f],nc_mcost[f]);
    }
  }
  if(particle->nPiminus()>0){
    for(int f=1; f<2; f++){
      ...
      FillHist(Form("pKn_pim_MMvsMomentum_%s",frame[f].Data()),nc_mmass[f],nc_mmom[f]);
      FillHist(Form("pKn_pim_MMvsCosT_%s",frame[f].Data()),nc_mmass[f],nc_mcost[f]);
    }
  }
  if(particle->nKaon()>0){
    for(int f=1; f<2; f++){
      ...
      FillHist(Form("pKn_k_MMvsMomentum_%s",frame[f].Data()),nc_mmass[f],nc_mmom[f]);
      FillHist(Form("pKn_k_MMvsCosT_%s",frame[f].Data()),nc_mmass[f],nc_mcost[f]);
    }
  }
  if(particle->nProton()>0){
    for(int f=1; f<2; f++){
      ...
      FillHist(Form("pKn_p_MMvsMomentum_%s",frame[f].Data()),nc_mmass[f],nc_mmom[f]);
      FillHist(Form("pKn_p_MMvsCosT_%s",frame[f].Data()),nc_mmass[f],nc_mcost[f]);
    }
  }
  if(particle->nPiplus()==1&&particle->nPiminus()==1){
    for(int f=1; f<2; f++){
      ...
      FillHist(Form("pKn_pipi_MMvsMomentum_%s",frame[f].Data()),nc_mmass[f],nc_mmom[f]);
      FillHist(Form("pKn_pipi_MMvsCosT_%s",frame[f].Data()),nc_mmass[f],nc_mcost[f]);
    }
  }
  if(k0flag){
    for(int f=1; f<2; f++){
  ...
  if(spflag){
    for(int f=1; f<2; f++){
      ...
      FillHist(Form("pKn_s_MMvsMomentum_%s",frame[f].Data()),nc_mmass[f],nc_mmom[f]);
      FillHist(Form("pKn_s_MMvsCosT_%s",frame[f].Data()),nc_mmass[f],nc_mcost[f]);
      if(!k0flag){
        FillHist(Form("pKn_sp_wok0_MMvsMomentum_%s",frame[f].Data()),nc_mmass[f],nc_mmom[f]);
        FillHist(Form("pKn_sp_wok0_MMvsCosT_%s",frame[f].Data()),nc_mmass[f],nc_mcost[f]);
        FillHist(Form("pKn_s_wok0_MMvsMomentum_%s",frame[f].Data()),nc_mmass[f],nc_mmom[f]);
        FillHist(Form("pKn_s_wok0_MMvsCosT_%s",frame[f].Data()),nc_mmass[f],nc_mcost[f]);
      }
    }
  }
  if(smflag){
    for(int f=1; f<2; f++){
      ...
      FillHist(Form("pKn_s_MMvsCosT_%s",frame[f].Data()),nc_mmass[f],nc_mcost[f]);
      if(!k0flag){
        FillHist(Form("pKn_sm_wok0_MMvsMomentum_%s",frame[f].Data()),nc_mmass[f],nc_mmom[f]);
        FillHist(Form("pKn_sm_wok0_MMvsCosT_%s",frame[f].Data()),nc_mmass[f],nc_mcost[f]);
        FillHist(Form("pKn_s_wok0_MMvsMomentum_%s",frame[f].Data()),nc_mmass[f],nc_mmom[f]);
        FillHist(Form("pKn_s_wok0_MMvsCosT_%s",frame[f].Data()),nc_mmass[f],nc_mcost[f]);
      }
    }
  ...
  for(int f=1; f<2; f++){
    ...
    FillHist(Form("pKn%s_MMvsMomentum_%s",pname[cds->pid()].Data(),frame[f].Data()),ncds_mmass[f],ncds_mmom[f]);
    FillHist(Form("pKn%s_MMvsCosT_%s",pname[cds->pid()].Data(),frame[f].Data()),ncds_mmass[f],ncds_mcost[f]);
  }
...
bool MyAnalysispKn::FillHist(TString name, double val1, int weight)
{
...
bool MyAnalysispKn::FillHist(TString name, TString val1, int weight)
{
  TH1F* h1 = (TH1F*)gFile -> Get(name);
  if(h1&&weight>0){
    for(int i=0; i<weight; i++){
...
bool MyAnalysispKn::FillHist(TString name, double val1, double val2, int weight)
{
...
bool MyAnalysispKn::FillHist(TString name, TString val1, TString val2, int weight)
{
  TH2F* h2 = (TH2F*)gFile -> Get(name);
  if(h2&&weight>0){
    for(int i=0; i<weight; i++){
...
void MyAnalysispKn::CutCondition()
{
  /* K0 mass cut */ 
  double mean  = 0.4976;
  double sigma = 0.0069;
...
bool MyAnalysispKn::Initialize(ConfMan* confMan)
{
  std::cout << "### MyAnalysispKn::Initialize ###" << std::endl;

  std::string ofname = confMan->GetOutFileName();
  ofname.insert(ofname.find(".root"),"_anapKn");
  ...
  for(int ip=0; ip<6; ip++){
    ...
    if(2<=ip&&ip<=4){
      new TH2F(Form("pKn_%s_wok0_MMvsMomentum_CM",proname[ip].Data()), "p(K^{-},n)X missing mass vs. missing momentum;MM_{p}(n) (GeV/c^{2});Missing momentum (GeV/c)", 2000, 0.0, 2.0, 1000, 0.0, 1.0);
      new TH2F(Form("pKn_%s_wok0_MMvsCosT_CM",proname[ip].Data()), "p(K^{-},n)X missing mass vs. cos#theta;MM_{p}(n) (GeV/c^{2});cos#theta", 2000, 0.0, 2.0, 2000, -1.0, 1.0);
    }
