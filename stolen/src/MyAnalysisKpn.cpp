...
MyAnalysisKpn::MyAnalysisKpn(TFile* rt, ConfMan* conf)
{
  Initialize(conf);
  CutCondition();
  Clear();
...
void MyAnalysisKpn::Clear()
{
}

bool MyAnalysisKpn::DoAnalysis(ConfMan* conf, EventHeader* header, BeamLineHitMan* blMan, BeamLineTrackMan* bltrackMan, CDSHitMan* cdsMan, CDSTrackingMan* cdstrackMan, Particle* particle)
{
  ...
  for(int it=0; it<particle->nProduct(); it++){
    ...
    if(cds->comb()!=pow(2,CDS_Proton)+pow(2,CDS_Kaon)){ continue; }
    fk0 = it;
  }
  pCDS* k0 = 0;
  ...
  FillHist(Form("kpn_MMass"),ncds_mmass[1]);
  FillHist(Form("kpn_MMom"),ncds_mmom[1]);
  FillHist(Form("kpn_MCosT"),ncds_mcost[1]);

  return true;

}

bool MyAnalysisKpn::FillHist(TString name, double val1, int weight)
{
...
bool MyAnalysisKpn::FillHist(TString name, TString val1, int weight)
{
  TH1F* h1 = (TH1F*)gFile -> Get(name);
  if(h1&&weight>0){
    for(int i=0; i<weight; i++){
...
bool MyAnalysisKpn::FillHist(TString name, double val1, double val2, int weight)
{
...
bool MyAnalysisKpn::FillHist(TString name, TString val1, TString val2, int weight)
{
  TH2F* h2 = (TH2F*)gFile -> Get(name);
  if(h2&&weight>0){
    for(int i=0; i<weight; i++){
...
void MyAnalysisKpn::CutCondition()
{
  /* K0 mass cut */ 
  double mean  = 0.4972;
  double sigma = 0.0060;
...
bool MyAnalysisKpn::Initialize(ConfMan* confMan)
{
  std::cout << "### MyAnalysisKpn::Initialize ###" << std::endl;

  std::string ofname = confMan->GetOutFileName();
  ofname.insert(ofname.find(".root"),"_anaKpn");
  ...
  TString pname[4]  = {"kpn","k0","sblk0","sbuk0"};
  TString pname2[4] = {"K^{-}pn","K^{0}","K^{0}_{lower}","K^{0}_{upper}"};
