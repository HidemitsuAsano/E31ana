...
bool EventAnalysisMyStart::UAna( TKOHitCollection *tko )
{
    ...
    for( int i=0; i<blMan->nT0(); i++ ){
      ...
      double time = (double)Event_Number/60.;
#if Debug
      std::cout << "= trg =" << std::endl;
      std::cout << "seg = " << seg << std::endl;
#endif
        h1 = (TH1F*)gFile->Get( Form("T0u%d_TDC",seg) ); h1->Fill( tu );
        h2 = (TH2F*)gFile->Get( Form("T0u%d_TDCvEvent",seg) ); h2->Fill( time,tu );
        h1 = (TH1F*)gFile->Get( Form("T0d%d_TDC",seg) ); h1->Fill( td );
        h2 = (TH2F*)gFile->Get( Form("T0d%d_TDCvEvent",seg) ); h2->Fill( time,td );
    }
    ...
    for( int i=0; i<blMan->nStart(); i++ ){
      ...
      std::cout << "seg = " << seg << std::endl;
        ...
        h2 = (TH2F*)gFile->Get( Form("Start%d_TDCvEvent",seg) ); h2->Fill( time,tu );
    }
...
void EventAnalysisMyStart::InitializeHistogram()
{
  ...
  for( int seg=1; seg<=5; seg++ ){
    ...
    new TH2F( Form("T0u%d_TDCvEvent",seg),   Form("TDC T0U%d;Time (min.);TDC ch.",seg),    600, 0, 60,  4001,    -0.5, 4000.5 );
    new TH2F( Form("T0d%d_TDCvEvent",seg),   Form("TDC T0D%d;Time (min.);TDC ch.",seg),    600, 0, 60, 4001,    -0.5, 4000.5 );
  }
  ...
  for( int seg=1; seg<=NumOfStartSegments; seg++ ){
    ...
    new TH2F( Form("Start%d_TDCvEvent",seg),   Form("%s;Time (min.);TDC ch.",startname[seg-1].Data()),    600, 0, 60, 4001,    -0.5, 4000.5 );
  }
