// MyAnalysisHeKpippOneDummyLpair.cpp

#include "MyAnalysisHeKpippOneDummyLpair.h"

MyAnalysisHeKpippOneDummyLpair::MyAnalysisHeKpippOneDummyLpair(TFile* rt, ConfMan* conf)
{
  Initialize(conf);
  CutCondition();
  Clear();
...
MyAnalysisHeKpippOneDummyLpair::~MyAnalysisHeKpippOneDummyLpair()
{
  eventdiff=0;
  Clear();
  rtFile->cd();
...
void MyAnalysisHeKpippOneDummyLpair::Clear()
{
}

bool MyAnalysisHeKpippOneDummyLpair::DoAnalysis(ConfMan* conf, EventHeader* header, BeamLineHitMan* blMan, BeamLineTrackMan* bltrackMan, CDSHitMan* cdsMan, CDSTrackingMan* cdstrackMan, Particle* particle)
{
  ...
  if(eventdiff<1){
    ...
    pCDS* tmp_pim = new pCDS(); tmp_pim = pim;
    pim = &cds_pim;
    cds_pim = *tmp_pim;
  }
  ...
  { /* pip1 */
    ...
    if(!TrackTools::Calc2HelixVertex(cdc1,cdc2,vtx1,vtx2)) return true;
    vtx = (vtx1+vtx2)*0.5;

    TVector3 Pp1=DEFVECT,Pp2=DEFVECT;
    if(!cdc1->GetMomentum(vtx1,Pp1,true,true)) return true;
    if(!cdc2->GetMomentum(vtx2,Pp2,true,true)) return true;
    TVector3 Pp = Pp1+Pp2;
    ...
    pip1->SetDaughterID1(cds1->id());
    pip1->SetDaughterID2(cds2->id());
    pip1->SetCombID((int)(pow(2,cds1->pid())+pow(2,cds2->pid())));
    pip1->SetMomentum(Pp.Mag());
    pip1->SetMass(im);
    pip1->SetVertex(vtx);
    pip1->SetVertexBeam(xest);
    pip1->SetMomDir(Pp.Unit());
    pip1->SetVDis((vtx1-vtx2).Mag());
    pip1->SetVBDis((xest-vtx).Mag());
    pip1->SetPBDCA(dist);
    pip1->SetOA(Pp1.Angle(Pp2));
  }
  pCDS* pip2 = new pCDS();
  { /* pip2 */
    pCDS* cds1 = pim;
    ...
    pip2->SetDaughterID1(cds1->id());
    pip2->SetDaughterID2(cds2->id());
    pip2->SetCombID((int)(pow(2,cds1->pid())+pow(2,cds2->pid())));
    pip2->SetMomentum(Pp.Mag());
    pip2->SetMass(im);
    pip2->SetVertex(vtx);
    pip2->SetVertexBeam(xest);
    pip2->SetMomDir(Pp.Unit());
    pip2->SetVDis((vtx1-vtx2).Mag());
    pip2->SetVBDis((xest-vtx).Mag());
    pip2->SetPBDCA(dist);
    pip2->SetOA(Pp1.Angle(Pp2));
  }
  ...
  double p_pipp_cost[2]  = { Lp.Vect().CosTheta()             , cLp_pipp.Vect().CosTheta()};
  ...
  double lam_pipp_cost[2]  = { Llam.Vect().CosTheta()         , cLlam_pipp.Vect().CosTheta()};
  ...
  // proton in Lp frame
  for(int f=1; f<2; f++){
    FillHist(Form("p_pipp_Momentum_%s",frame[f].Data()),p_pipp_mom[f]);
    FillHist(Form("p_pipp_CosT_%s",frame[f].Data()),p_pipp_cost[f]);
    FillHist(Form("p_pipp_Phi_%s",frame[f].Data()),p_pipp_phi[f]);
  }
  // Lambda in Lp frame
  for(int f=1; f<2; f++){
    FillHist(Form("lam_pipp_Mass_%s",frame[f].Data()),lam_pipp_mass[f]);
    FillHist(Form("lam_pipp_Momentum_%s",frame[f].Data()),lam_pipp_mom[f]);
    FillHist(Form("lam_pipp_CosT_%s",frame[f].Data()),lam_pipp_cost[f]);
    FillHist(Form("lam_pipp_Phi_%s",frame[f].Data()),lam_pipp_phi[f]);
  }
  // proton1
  for(int f=1; f<2; f++){
...
bool MyAnalysisHeKpippOneDummyLpair::FillHist(TString name, double val1, int weight)
{
...
bool MyAnalysisHeKpippOneDummyLpair::FillHist(TString name, TString val1, int weight)
{
  TH1F* h1 = (TH1F*)gFile -> Get(name);
  if(h1&&weight>0){
    for(int i=0; i<weight; i++){
...
bool MyAnalysisHeKpippOneDummyLpair::FillHist(TString name, double val1, double val2, int weight)
{
...
bool MyAnalysisHeKpippOneDummyLpair::FillHist(TString name, TString val1, TString val2, int weight)
{
  TH2F* h2 = (TH2F*)gFile -> Get(name);
  if(h2&&weight>0){
    for(int i=0; i<weight; i++){
...
void MyAnalysisHeKpippOneDummyLpair::CutCondition()
{
  /* K0 mass cut */ 
  double mean  = 0.4976;
  double sigma = 0.0069;
...
bool MyAnalysisHeKpippOneDummyLpair::Initialize(ConfMan* confMan)
{
  std::cout << "### MyAnalysisHeKpippOneDummyLpair::Initialize ###" << std::endl;

  std::string ofname = confMan->GetOutFileName();
  ofname.insert(ofname.find(".root"),"_anaHeKpippOneDummyLpair");
