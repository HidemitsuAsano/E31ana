...
bool MyAnalysisBLHodoCheck::DoAnalysis(ConfMan* conf, EventHeader* header, BeamLineHitMan* blMan, BeamLineTrackMan* bltrackMan)
{
  ...
  for(int i=0; i<blMan->nT0(); i++){
    if(blMan->T0(i)->CheckRange()){
      ...
      T0Time[0] = blMan->T0(i)->ctime(0);
      T0Time[1] = blMan->T0(i)->ctime(1);
  ...
  const int nhodo = 7;
  int hodoid[nhodo]={
    CID_BHD,CID_T0,CID_E0,CID_DEF,CID_BVC,
    CID_Start,CID_Stop
  };
  for(int ihodo=0; ihodo<nhodo; ihodo++){
    ...
    for( int i=0; i<blMan->nHodo(cid); i++ ){
      ...
      double tofu = ctmean - T0Time[0];
      double tofd = ctmean - T0Time[1];
      if(cid==CID_BHD){
        tof *= -1.0;
        udtof[0] *= -1.0;
        udtof[1] *= -1.0;
      ...
      if( hit->CheckRange() ){
        for(int iud=0; iud<2; iud++){
          ...
          if(-0.1<tsub&&tsub<0.1){
            ...
            if(cid!=CID_T0){
              ...
              FillHist(Form("%s%s%d_dEvsTOF",name,ud[iud],seg),ene[iud],udtof[iud],1);
            }
          }
        }
        ...
        nhit++;
        if(cid==CID_BHD){
          FillHist(Form("T0u%d_dEvsTOF",T0Segment),T0udE,tofu,1);
          FillHist(Form("T0d%d_dEvsTOF",T0Segment),T0ddE,tofd,1);
