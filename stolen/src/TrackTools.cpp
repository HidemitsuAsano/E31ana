...
bool TrackTools::CalcLineHelixVertex(TVector3 lpos,TVector3 ldir,CDSTrack *cds, TVector3 &vtx1, TVector3 &vtx2, double &dis)
{
  double par[5];
  TVector3 tmpvtx;
...
int TrackTools::PIDcorr2(double mom,double mass2)
{
  int ptype=CDS_Other;
  double fmom=fabs(mom);
  //double param[4][4]={{0.00227537,0.000112426,piMass*piMass,0.0130341},
...
int TrackTools::PIDcorr(double mom,double mass2)
{
  ...
  TF1 *f_sigma=new TF1("f_sigma","sqrt(4*[2]*[0]*x*x+4*[2]*[2]*[1]*(1+[2]/(x*x))+4*[3]*x*x*([2]+x*x))");
  double param[3][4]={{0.016119,0.000119028,0.0360037,piMass},
    {0.016119,0.000119028,0.0360037,kpMass},
    {0.016119,0.000119028,0.0360037,pMass}};
...
pCDS* TrackTools::CalcSingleAll(pBeam *beam,int trackID,CDSTrack* cdc,CDSHitMan *cdsMan,bool ELOSS)
{
  double cdhtime,dis;
  int cdhseg;
  ...
  cdstrack->SetTrackID(trackID);      /* Added by TY (2015.07.02) */
  ...
  double cdcdt=(time_cdc+tofvtxcdc+time_beam);
...
{
  CDSTrack *track1=trackMan->GoodTrack(cds1->id());	
  CDSTrack *track2=trackMan->GoodTrack(cds2->id());	
...
void TrackTools::PDFLambda(double* per, double* pdf){
  ...
  c1 = 1.1155;
  c2 = 0.002;
  tmppdf = c0 * TMath::Exp(-(per[0]-c1)*(per[0]-c1)/2.0/c2/c2);
  pdf[0] = tmppdf;
  //std::cout << "per0 : " << per[0] <<  " --> " << tmppdf << std::endl;
  /* DCA1 (pi-p)*/
  /* 2017.08.10 */
  c0 =  1.0;
  c1 = -1.578198;
  ...
  //c4 = -1.795197;
  //c5 = -3.586255;
  //tmppdf = c0*TMath::Exp(c1*per[1]) + c2*TMath::Exp(c3*TMath::Power((per[1]-c4),c5));
  tmppdf = c0*TMath::Exp(c1*per[1]+c2*per[1]*per[1]);
  pdf[1] = tmppdf;
  ///* DCA2 (Lambda-K)*/
  /* 2017.08.10 */
  c0 =  1.0;
  ...
  //c4 = -6.068499; 
  //c5 = -1.703870;
  //tmppdf = c0*TMath::Exp(c1*per[2]) + c2*TMath::Exp(c3*TMath::Power((per[2]-c4),c5));
  tmppdf = c0*TMath::Exp(c1*per[2]+c2*per[2]*per[2]);
  pdf[2] = tmppdf;
  ///* DCA3 (p-K)*/
  /* 2017.08.10 */
  c0 =  1.0;
  ...
  //c4 = -4.465181; 
  //c5 = -1.910207;
  //tmppdf = c0*TMath::Exp(c1*per[3]) + c2*TMath::Exp(c3*TMath::Power((per[3]-c4),c5));
  tmppdf = c0*TMath::Exp(c1*per[3]+c2*per[3]*per[3]);
  pdf[3] = tmppdf;
  /* DCA4 (Lambda-p)*/
	/* 2017.08.10 */
  c0 =  1.0;
