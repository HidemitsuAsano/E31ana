// MyAnalysisHeKFWDNeutral.cpp

#include "MyAnalysisHeKFWDNeutral.h"

MyAnalysisHeKFWDNeutral::MyAnalysisHeKFWDNeutral(TFile* rt, ConfMan* conf)
{
  Initialize(conf);
  CutCondition();
  Clear();
...
MyAnalysisHeKFWDNeutral::~MyAnalysisHeKFWDNeutral()
{
  Clear();
  rtFile->cd();
  rtFile->Write();
...
void MyAnalysisHeKFWDNeutral::Clear()
{
}

bool MyAnalysisHeKFWDNeutral::DoAnalysis(ConfMan* conf, EventHeader* header, BeamLineHitMan* blMan, BeamLineTrackMan* bltrackMan, CDSHitMan* cdsMan, CDSTrackingMan* cdstrackMan, Particle* particle)
{
  rtFile->cd();
  ...
  pCDS* cds2=0;
  if(fcds!=-1){
    cds1 = particle->proton(fcds);
    if(particle->nProton()==2){
      cds2 = particle->proton(1-fcds);
    }
  }
...
bool MyAnalysisHeKFWDNeutral::DoOneProtonAna(Particle* particle, pBeam* beam, pCDS* cds, TVector3 vertex)
{
  TString mode = "1p";
...
bool MyAnalysisHeKFWDNeutral::DoTwoProtonAna(Particle* particle, pBeam* beam, pCDS* cds1, pCDS* cds2, TVector3 vertex)
{
  TString mode = "2p";
...
bool MyAnalysisHeKFWDNeutral::FillHist(TString name, double val1, int weight)
{
...
bool MyAnalysisHeKFWDNeutral::FillHist(TString name, TString val1, int weight)
{
  TH1F* h1 = (TH1F*)gFile -> Get(name);
  if(h1&&weight>0){
    for(int i=0; i<weight; i++){
...
bool MyAnalysisHeKFWDNeutral::FillHist(TString name, double val1, double val2, int weight)
{
...
bool MyAnalysisHeKFWDNeutral::FillHist(TString name, TString val1, TString val2, int weight)
{
  TH2F* h2 = (TH2F*)gFile -> Get(name);
  if(h2&&weight>0){
    for(int i=0; i<weight; i++){
...
void MyAnalysisHeKFWDNeutral::CutCondition()
{
  /* K0 mass cut */ 
  double mean  = 0.4976;
  double sigma = 0.0069;
...
bool MyAnalysisHeKFWDNeutral::Initialize(ConfMan* confMan)
{
  std::cout << "### MyAnalysisHeKFWDNeutral::Initialize ###" << std::endl;

  std::string ofname = confMan->GetOutFileName();
  ofname.insert(ofname.find(".root"),"_anaHeKFWDNeutral");
