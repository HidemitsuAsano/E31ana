// MyAnalysisHeKpippCheckResl.cpp

#include "MyAnalysisHeKpippCheckResl.h"

MyAnalysisHeKpippCheckResl::MyAnalysisHeKpippCheckResl(TFile* rt, ConfMan* conf)
{
  Initialize(conf);
  CutCondition();
  Clear();
...
MyAnalysisHeKpippCheckResl::~MyAnalysisHeKpippCheckResl()
{
  Clear();
  rtFile->cd();
  rtFile->Write();
...
void MyAnalysisHeKpippCheckResl::Clear()
{
}

bool MyAnalysisHeKpippCheckResl::DoAnalysis(ConfMan* conf, EventHeader* header, BeamLineHitMan* blMan, BeamLineTrackMan* bltrackMan, CDSHitMan* cdsMan, CDSTrackingMan* cdstrackMan, Particle* particle, MCData* mcdata, ReactionData* reaction, DetectorData* detector)
{
  ...
  for(int i=0; i<reaction->ParticleSize(); i++){
    if(reaction->PDG(i)==3122||reaction->PDG(i)==3212){
  ...
  //Llam.SetVectM(Llam.Vect(),1.115683);
  TLorentzVector cLlam = lam->GetLorentzVector();
  //cLlam.SetVectM(cLlam.Vect(),1.115683);
  cLlam.Boost(-boost);
  TLorentzVector Lnlam = nlam->GetLorentzVector();
  //Lnlam.SetVectM(Lnlam.Vect(),1.115683);
  TLorentzVector cLnlam = nlam->GetLorentzVector();
  //cLnlam.SetVectM(cLnlam.Vect(),1.115683);
  cLnlam.Boost(-boost);
  ...
  //Lmn.SetVectM(Lmn.Vect(),0.939565);
  TLorentzVector cLmn = cLtgt+cLbeam-cLpipp;
  //cLmn.SetVectM(cLmn.Vect(),0.939565);
  ...
  TVector3 boost_pipp = (Lpipp).BoostVector();
  TLorentzVector cLp_pipp = Lp; cLp_pipp.Boost(-boost_pipp);
  TLorentzVector cLlam_pipp = Llam; cLlam_pipp.Boost(-boost_pipp);
  double p_pipp_mass[2]  = { Lp.M()                           , cLp_pipp.M()};
  double p_pipp_mom[2]   = { Lp.Vect().Mag()                  , cLp_pipp.Vect().Mag()};
  double p_pipp_cost[2]  = { Lp.Vect().Dot(Lbeam.Vect())/Lp.Vect().Mag()/Lbeam.Vect().Mag(),cLp.Vect().Dot(cLbeam.Vect())/cLp.Vect().Mag()/cLbeam.Vect().Mag()};
  double p_pipp_phi[2]   = { Lp.Vect().Phi()                  , cLp_pipp.Vect().Phi()};
  double lam_pipp_mass[2]  = { Llam.M()                       , cLlam_pipp.M()};
  double lam_pipp_mom[2]   = { Llam.Vect().Mag()              , cLlam_pipp.Vect().Mag()};
  double lam_pipp_cost[2]  = { Llam.Vect().Dot(Lbeam.Vect())/Llam.Vect().Mag()/Lbeam.Vect().Mag(),cLlam.Vect().Dot(cLbeam.Vect())/cLlam.Vect().Mag()/cLbeam.Vect().Mag()};
  double lam_pipp_phi[2]   = { Llam.Vect().Phi()              , cLlam_pipp.Vect().Phi()};
  ...
  for(int f=1; f<2; f++){
    ...
    FillHist(Form("p_MomentumDiff_%s",frame[f].Data()),(p_mom[f]-p_mc_mom[f])*1000.);
    FillHist(Form("p_MMassDiff_%s",frame[f].Data()),(p_mmass[f]-p_mc_mmass[f])*1000.);
  }
  // proton1
  for(int f=1; f<2; f++){
  ...
  for(int f=1; f<2; f++){
    ...
    FillHist(Form("lam_MassDiff_%s",frame[f].Data()),(lam_mass[f]-lam_mc_mass[f])*1000.0);
    FillHist(Form("lam_MomentumDiff_%s",frame[f].Data()),(lam_mom[f]-lam_mc_mom[f])*1000.0);
    FillHist(Form("lam_MMassDiff_%s",frame[f].Data()),(lam_mmass[f]-lam_mc_mmass[f])*1000.0);
  }
  // nlambda
  for(int f=1; f<2; f++){
  ...
  for(int f=1; f<2; f++){
    ...
    FillHist(Form("pipp_MassDiff_%s",frame[f].Data()),(pipp_mass[f]-plam_mc_mass[f])*1000.0);
    FillHist(Form("pipp_MomentumDiff_%s",frame[f].Data()),(pipp_mom[f]-plam_mc_mom[f])*1000.0);
    FillHist(Form("pipp_MMassDiff_%s",frame[f].Data()),(pipp_mmass[f]-plam_mc_mmass[f])*1000.0);
    FillHist(Form("pipp_Mass_vs_pipp_MassDiff_%s",frame[f].Data()),pipp_mass[f],(pipp_mass[f]-plam_mc_mass[f])*1000.0);
  }
  ...
  // IM(pipp) vs. Phi(lan) in Lp
  for(int f=1; f<2; f++){
    FillHist(Form("pipp_IMvslam_pipp_Phi_%s",frame[f].Data()),pipp_mass[f],lam_pipp_phi[f]);
  }
  // MCosT(pipp) vs. Phi(lan) in Lp
  for(int f=1; f<2; f++){
    FillHist(Form("pipp_MCosTvslam_pipp_Phi_%s",frame[f].Data()),pipp_mcost[f],lam_pipp_phi[f]);
  }
...
bool MyAnalysisHeKpippCheckResl::FillHist(TString name, double val1, int weight)
{
...
bool MyAnalysisHeKpippCheckResl::FillHist(TString name, TString val1, int weight)
{
  TH1F* h1 = (TH1F*)gFile -> Get(name);
  if(h1&&weight>0){
    for(int i=0; i<weight; i++){
...
bool MyAnalysisHeKpippCheckResl::FillHist(TString name, double val1, double val2, int weight)
{
...
bool MyAnalysisHeKpippCheckResl::FillHist(TString name, TString val1, TString val2, int weight)
{
  TH2F* h2 = (TH2F*)gFile -> Get(name);
  if(h2&&weight>0){
    for(int i=0; i<weight; i++){
...
void MyAnalysisHeKpippCheckResl::CutCondition()
{
  /* K0 mass cut */ 
  double mean  = 0.4976;
  double sigma = 0.0069;
...
bool MyAnalysisHeKpippCheckResl::Initialize(ConfMan* confMan)
{
  std::cout << "### MyAnalysisHeKpippCheckResl::Initialize ###" << std::endl;

  std::string ofname = confMan->GetOutFileName();
  ofname.insert(ofname.find(".root"),"_anaHeKpippCheckResl");
  ...
  new TH1F("p_MomentumDiff_CM","Momentum of p;Momentum (MeV/c);Counts", 5000, -250.0, 250.0);
  new TH1F("p_MMassDiff_CM", "^{3}He(K^{-},p)X missing mass;MM(p) (MeV/c^{2});Counts", 5000, -250.0, 250.0);
  // proton in Lp
  ...
  new TH1F("lam_MassDiff_CM","Invariant mass of #Lambda;IM(#Lambda) (MeV/c^{2});Counts", 5000, -250.0, 250.0);
  new TH1F("lam_MomentumDiff_CM","Momentum of #Lambda;Momentum (MeV/c);Counts", 5000, -250.0, 250.0);
  new TH1F("lam_MMassDiff_CM", "^{3}He(K^{-},#Lambda)X missing mass;MM(#Lambda) (MeV/c^{2});Counts", 5000, -250.0, 250.0);
  // Lambda in Lp
  ...
  new TH1F("pipp_MassDiff_CM","Invariant mass of #pi^{-}pp;IM(#pi^{-}pp) (MeV/c^{2});Counts", 5000, -250.0, 250.0);
  new TH1F("pipp_MomentumDiff_CM","Momentum of #pi^{-}pp;Momentum (MeV/c);Counts", 5000, -250.0, 250.0);
  new TH1F("pipp_MMassDiff_CM", "^{3}He(K^{-},#pi^{-}pp)X missing mass;MM(#pi^{-}pp) (MeV/c^{2});Counts", 5000, -250.0, 250.0);
  // Not pi-,proton1,proton2
  ...
  /* 2D Plots */
  new TH2F("pipp_Mass_vs_pipp_MassDiff_CM", "(#pi^{-}p_{1}p_{2}) invariant mass vs. #phi_{#Lambda}^{#Lambdap};IM(#pi^{-}p_{1}p_{2}) (GeV/c^{2});Mass Diff", 200, 2.0, 3.0, 600, -150, 150);
