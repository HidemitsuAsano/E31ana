// MyAnalysisHeKpippOneDummy.cpp

#include "MyAnalysisHeKpippOneDummy.h"

MyAnalysisHeKpippOneDummy::MyAnalysisHeKpippOneDummy(TFile* rt, ConfMan* conf)
{
  eventdiff = 0;
  Initialize(conf);
  CutCondition();
...
MyAnalysisHeKpippOneDummy::~MyAnalysisHeKpippOneDummy()
{
  Clear();
  rtFile->cd();
  rtFile->Write();
...
void MyAnalysisHeKpippOneDummy::Clear()
{
}

bool MyAnalysisHeKpippOneDummy::DoAnalysis(ConfMan* conf, EventHeader* header, BeamLineHitMan* blMan, BeamLineTrackMan* bltrackMan, CDSHitMan* cdsMan, CDSTrackingMan* cdstrackMan, Particle* particle)
{
...
bool MyAnalysisHeKpippOneDummy::FillHist(TString name, double val1, int weight)
{
...
bool MyAnalysisHeKpippOneDummy::FillHist(TString name, TString val1, int weight)
{
  TH1F* h1 = (TH1F*)gFile -> Get(name);
  if(h1&&weight>0){
    for(int i=0; i<weight; i++){
...
bool MyAnalysisHeKpippOneDummy::FillHist(TString name, double val1, double val2, int weight)
{
...
bool MyAnalysisHeKpippOneDummy::FillHist(TString name, TString val1, TString val2, int weight)
{
  TH2F* h2 = (TH2F*)gFile -> Get(name);
  if(h2&&weight>0){
    for(int i=0; i<weight; i++){
...
void MyAnalysisHeKpippOneDummy::CutCondition()
{
  /* K0 mass cut */ 
  double mean  = 0.4976;
  double sigma = 0.0069;
...
bool MyAnalysisHeKpippOneDummy::Initialize(ConfMan* confMan)
{
  std::cout << "### MyAnalysisHeKpippOneDummy::Initialize ###" << std::endl;

  std::string ofname = confMan->GetOutFileName();
  ofname.insert(ofname.find(".root"),"_anaHeKpippOneDummy");
