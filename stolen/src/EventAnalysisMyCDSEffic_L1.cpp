...
void EventAnalysisMyCDC::InitializeHistogram()
{
	...
	for(int idc=0;idc<ndc;idc++){
		...
		for( int layer=1; layer<=NumOfLayers[idc]; layer++ ){
			...
			new TH1F( Form("%s_Layer%d_Distance",dcname[idc].Data(),layer), Form("Residual %s Layer%d;Distance (mm);Counts",dcname[idc].Data(),layer), 10000, 0.0, 1000.0 );
			new TH1F( Form("%s_Layer%d_Distance_Closed",dcname[idc].Data(),layer), Form("Residual %s Layer%d;Distance (mm);Counts",dcname[idc].Data(),layer), 10000, 0.0, 1000.0 );
