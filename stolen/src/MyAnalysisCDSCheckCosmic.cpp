// MyAnalysisCDSCheckCosmic.cpp

#include "MyAnalysisCDSCheckCosmic.h"

MyAnalysisCDSCheckCosmic::MyAnalysisCDSCheckCosmic(TFile* rt, ConfMan* conf)
{
  Initialize(conf);
  CutCondition();
  Clear();
...
MyAnalysisCDSCheckCosmic::~MyAnalysisCDSCheckCosmic()
{
  Clear();
  rtFile->cd();
  rtFile->Write();
...
void MyAnalysisCDSCheckCosmic::Clear()
{
}

bool MyAnalysisCDSCheckCosmic::DoAnalysis(ConfMan* conf, EventHeader* header, BeamLineHitMan* blMan, CDSHitMan* cdsMan, CDSTrackingMan* cdstrackMan)
{
...
bool MyAnalysisCDSCheckCosmic::FillHist(TString name, double val1, int weight)
{
...
bool MyAnalysisCDSCheckCosmic::FillHist(TString name, TString val1, int weight)
{
  TH1F* h1 = (TH1F*)gFile -> Get(name);
  if(h1&&weight>0){
    for(int i=0; i<weight; i++){
...
bool MyAnalysisCDSCheckCosmic::FillHist(TString name, double val1, double val2, int weight)
{
...
bool MyAnalysisCDSCheckCosmic::FillHist(TString name, TString val1, TString val2, int weight)
{
  TH2F* h2 = (TH2F*)gFile -> Get(name);
  if(h2&&weight>0){
    for(int i=0; i<weight; i++){
...
void MyAnalysisCDSCheckCosmic::CutCondition()
{
}

double MyAnalysisCDSCheckCosmic::CalcTimeOffs(HodoscopeLikeHit* hit, Particle* particle)
{
...
double MyAnalysisCDSCheckCosmic::CalcTimeSubOffs(HodoscopeLikeHit* hit, Particle* particle)
{
  double tmpoffset = -9999.9;
  if(!hit->CheckRange()) return tmpoffset;
  pBeam* beam = particle->beam(0);
...
bool MyAnalysisCDSCheckCosmic::Initialize(ConfMan* confMan)
{
  std::cout << "### MyAnalysisCDSCheckCosmic::Initialize ###" << std::endl;

  confFile = confMan;
  DetectorList *dlist=DetectorList::GetInstance();

  std::string ofname = confMan->GetOutFileName();
  ofname.insert(ofname.find(".root"),"_anaCDSCheckCosmic");
  ...
  for( int layer=1; layer<=nlays; layer++ ){
    ...
    new TH2F( Form("%sl%d_TimevsResidualinTrack",name,layer),Form("Time vs. Residual %s-L%d;%s (layer %d) Time (ns);Residual (cm)",name,layer,name,layer),800,-400,400,2000,-0.2,0.2);
    ...
    for(int wire=1;wire<=nwire;wire++){
      ...
      new TH1F( Form("%sl%dw%02d_TimeinTrack",name,layer,wire),Form("Time %s-L%dW%02d;%s (layer %d,wire %d) Time (ns);Counts",name,layer,wire,name,layer,wire),310,-10,300);
      new TH1F( Form("%sl%dw%02d_ResidualinTrack",name,layer,wire),Form("Residual %s-L%dW%02d;%s (layer %d,wire %d) Residual (cm);Counts",name,layer,wire,name,layer,wire),2000,-0.2,0.2);
      new TH2F( Form("%sl%dw%02d_TimevsResidualinTrack",name,layer,wire),Form("Time vs. Residual %s-L%dW%02d;%s (layer %d,wire %d) Time (ns);%s (layer %d,wire %d) Residual (cm)",name,layer,wire,name,layer,wire,name,layer,wire),310,-10,300,1000,-0.2,0.2);
    }
  }
