...
void HistManBeamAna::fillFC(ConfMan *conf, EventHeader *header, BeamLineHitMan *blMan, BeamLineTrackMan *bltrackMan, CDSHitMan *cdsMan, CDSTrackingMan *cdstrackMan)
{
  ...
  // if( header-> trigmode2(Mode_KCDH1N, conf) ){
  //   fNumNEv++;
  //   std::cout<<"===== TirgMode : KCDH1N ====="<<std::endl;
  //   if( fBeamPID==Beam_Kaon ) std::cout<<"> Beam Kaon"<<std::endl;
  //   else if( fBeamPID==Beam_Pion ) std::cout<<"> Beam Pion"<<std::endl;
  //   else if( fBeamPID==Beam_Other ) std::cout<<"> unknown Beam"<<std::endl;

  //   if( fBeamMom>0.001 ) std::cout<<"> Beam Mom : "<<fBeamMom<<"[GeV/c]"<<std::endl;
  //   else std::cout<<"> Beam Mom can not be calcrated"<<std::endl;

  //   std::cout<<"> BPC  ntrack : "<<bltrackMan->ntrackBPC()<<std::endl;
  //   std::cout<<"> FDC1 ntrack : "<<bltrackMan->ntrackFDC1()<<std::endl;

  //   std::cout<<"> CVC hit : "<<fCVC_hit.size()<<std::endl;
  //   bool NC_flag = false;
  //   for( int lay=0; lay<8; lay++ ){
  //     if( fNC_hit[lay].size()>0 )  NC_flag = true;
  //     std::cout<<"> NC lay"<<lay<<" hit : "<<fNC_hit[lay].size()<<std::endl;
  //   }
  //   if( fBeamPID==Beam_Kaon && fBeamMom>0.001 && fCVC_hit.size()==0 && NC_flag ){
  //     fGoodNEv++;
  //   }
  //   std::cout<<"> Good/N Trig="<<fGoodNEv<<"/"<<fNumNEv<<std::endl;
  // }
	...
	if( fCVC_hit.size()==0 ){
	  ...
	  for( int lay=0; lay<8; lay++ ){
	    for( int i=0; i<fNC_hit[lay].size(); i++ ){
	      ...
	      fSlewingT0NC-> Set(fT0_hit[0], fNC_hit[lay][0]);
		...
		if( fBeamPID==Beam_Kaon && header-> trigmode2(Mode_KCDH1N) ){
		  ...
		  for( int i=0; i<fCDSpip.size(); i++ ){
			...
			if( MyTools::IsTarget(vtx) ){
			  ...
			  h1 = getTH("Npip_IM_wtar"), h1-> Fill(mm2);
			}
		      }
		    }
		  }
