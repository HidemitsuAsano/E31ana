...
bool MyAnalysisBL::DoAnalysis(ConfMan* conf, EventHeader* header, BeamLineHitMan* blMan, BeamLineTrackMan* bltrackMan, Particle* particle, double _mom)
{
	...
	for(int i=0; i<bltrackMan->ntrackBLC1(); i++){
		...
		for(int ihit=0; ihit<track->nhit(); ihit++){
			...
			if(TMath::Abs(maxresl1)<TMath::Abs(resl)){ maxresl1=resl; maxlayer=layer; maxcid=cid; }
		}
...
#endif
	ntrack_first[0] = bltrackMan->ntrackBLC2();
...
#endif
	ntrack_first[0] = bltrackMan->ntrackBPC();
	for(int i=0; i<bltrackMan->ntrackBPC(); i++){
		...
		BLDCParam->SetLayerDeadBPCa(maxlayer-1,true);
	}
#ifdef DEBUG
	std::cout << "------------------" << std::endl;
#endif
	...
	BLDCFittingParamMan* BLDCParam = conf->GetBLDCFittingParamManager();
	{
		...
		for(idc=0;idc<2;idc++){
			...
			double maxresl1=0.0, maxresl2=0.0;
			double chi2_1=-1, chi2_2=-1;
...
#endif
				FillHist(Form("%s_MaxResidual_vs_Chi2_First",dcname),maxresl1,chi2);
				if(maxcid==CID_BLC1a){BLDCParam->SetLayerDeadBLC1a(maxlayer-1,true);}
				if(maxcid==CID_BLC1b){BLDCParam->SetLayerDeadBLC1b(maxlayer-1,true);}
				if(maxcid==CID_BLC2a){BLDCParam->SetLayerDeadBLC2a(maxlayer-1,true);}
				if(maxcid==CID_BLC2b){BLDCParam->SetLayerDeadBLC2b(maxlayer-1,true);}
...
#endif
					if(dccid[idc]==CID_BLC1){bltrackMan->DeleteTrackBLC1(i);}
					else {bltrackMan->DeleteTrackBLC2(i);}
					i--;
				}
			}
			...
			std::cout << "ntrack = " << bltrackMan->ntrackBLDC(dccid[idc]) << std::endl;
			int n_second = bltrackMan->ntrackBLDC(dccid[idc]);
			if(n_first>n_second){ std::cout <<"!!!!!!!!!! Track Disappear !!!!!!!!!!" << std::endl;}
#endif
			...
			for(int i=0; i<bltrackMan->ntrackBLDC(dccid[idc]); i++){
				...
				for(int ihit=0; ihit<track->nhit(); ihit++){
					...
					int layer = hit->layer();
					double resl = hit->resl();
					if(TMath::Abs(maxresl2)<TMath::Abs(resl)){ maxresl2=resl; maxlayer=layer; }
				...
				std::cout << "track # = " << i <<  " --- " << chi2 << std::endl;
#endif
				FillHist(Form("%s_MaxResidual_vs_Chi2_Second",dcname),maxresl2,chi2);
			...
			FillHist(Form("%s_nGoodTrack",dcname),nhitdc[idc]);
			if(bltrackMan->ntrackBLDC(dccid[idc])==1&&nhitdc[idc]==1){
				FillHist(Form("%s_MaxResl1_vs_MaxResl2",dcname),maxresl1,maxresl2);
				FillHist(Form("%s_Chi21_vs_Chi22",dcname),chi2_1,chi2_2);
				if(dccid[idc]==CID_BLC1){
					//std::cout << "BLC1--- " << chi2_1 << " >> " << chi2_2 << std::endl;
				}
				else if(dccid[idc]==CID_BLC2){
					//std::cout << "BLC2--- " << chi2_1 << " >> " << chi2_2 << std::endl;
				}
			}
		}
		for(int i=0; i<8; i++){
		...
		if(nhitdc[0]==1&&nhitdc[1]==1){
			...
			BLC1Chi2 = blc1->chi2all();
			BLC1Prob = TMath::Prob(blc1->chi2all()*(double)(blc1->dofxz()+blc1->dofyz()),blc1->dofxz()+blc1->dofyz());
			BLC2Chi2 = blc2->chi2all();
			BLC2Prob = TMath::Prob(blc2->chi2all()*(double)(blc2->dofxz()+blc2->dofyz()),blc2->dofxz()+blc2->dofyz());
		...
		for(int i=0; i<bltrackMan->ntrackBLDC(dccid[idc]); i++){
			...
			for(int ihit=0; ihit<track->nhit(); ihit++){
				...
				int layer = hit->layer();
				double resl = hit->resl();
				if(TMath::Abs(maxresl1)<TMath::Abs(resl)){ maxresl1=resl; maxlayer=layer; }
			}
			double tracktime = track -> GetTrackTime();
			double chi2 = track -> chi2all();
			int dof = track->dofxz()+track->dofyz();
			double prob = TMath::Prob(chi2*(double)dof,dof);
			FillHist(Form("%s_MaxResidual_vs_Chi2_First",dcname),maxresl1,chi2);
			BLDCParam->SetLayerDeadBPC(maxlayer-1,true);
			if(ll[idc]<tracktime&&tracktime<ul[idc]){
				chi2_1 = chi2;
			}
		}
		...
		if(bltrackMan->ntrackBLDC(dccid[idc])==1&&nhitdc[idc]==1){
			FillHist(Form("%s_MaxResl1_vs_MaxResl2",dcname),maxresl1,maxresl2);
			FillHist(Form("%s_Chi21_vs_Chi22",dcname),chi2_1,chi2_2);
			std::cout << "BPC --- " << chi2_1 << " >> " << chi2_2 << std::endl;
		}
		for(int i=0; i<8; i++){
		...
		if(nhitdc[idc]==1){
			...
			double tracktime = track -> GetTrackTime();
			BPCChi2 = track->chi2all();
			BPCProb = TMath::Prob(track->chi2all()*(double)(track->dofxz()+track->dofyz()),track->dofxz()+track->dofyz());
...
bool MyAnalysisBL::Initialize(TFile* rtFile, ConfMan* confMan)
{
	...
	for(int idc=0;idc<ndc;idc++){
		...
		new TH2F( Form("%s_MaxResidual_vs_Chi2_First",dcname[idc].Data()),Form("Maximum residual vs. #chi^{2} %s;residual (cm);#chi^{2}",dcname[idc].Data()),500,-0.5,0.5,500,0.0,100.0);
		new TH2F( Form("%s_MaxResidual_vs_Chi2_Second",dcname[idc].Data()),Form("Maximum residual vs. #chi^{2} %s;residual (cm);#chi^{2}",dcname[idc].Data()),500,-0.5,0.5,500,0.0,100.0);
		new TH2F( Form("%s_Chi21_vs_Chi22",dcname[idc].Data()),Form("1st #chi^{2} vs. 2nd #chi^{2} %s;1st #chi^{2};2nd #chi^{2}",dcname[idc].Data()),500,0.0,100.0,500,0.0,100.0);
