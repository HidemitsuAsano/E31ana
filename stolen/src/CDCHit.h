...
class CDCHit : public ChamberLikeHit
{
 ...
 private:
  ...
  TVector3 WirePosp;// wire position opposite side of readout
 ...
 public:
  ...
  TVector3 wposp()   const { return WirePosp; }
  double wxp()    const { return WirePosp.X(); }
  double wyp()    const { return WirePosp.Y(); }
  double wzp()    const { return WirePosp.Z(); }
  ...
  { WirePosp.SetXYZ(x,y,z); }

  bool Calc( ConfMan *conf, const double &toffset=0.);
