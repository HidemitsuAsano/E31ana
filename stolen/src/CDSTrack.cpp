...
//*********Circle Fitng2************//
bool CDSTrack::HelixFitting(CDSHitMan *cdsMan,int l,int w)
{
	int allhit=0;
	...
	for(int layer=1;layer<=15;layer++) {
		for(int n=0;n<(int)nTrackHit(layer);n++) {
			...
			if(layer==l&&cdc->wire()==w){
				weight[allhit]=pow(10,300);
			}
			else{
...
//CalcShortInitial
bool CDSTrack::Retiming(CDSHitMan *cdsMan, ConfMan *conf,double beta,bool SLEW,int l, int w)
{
	double cdhtime;
	int seg;
	...
	for(int layer=1;layer<=15;layer++)
	{
		for(int n=0;n<(int)nTrackHit(layer);n++)
		{
			...
			//ctime = cdhtime-timeminus-5;
			if(SLEW){
				ctime = conf->GetSlewingMapManager()->CalcCValue(CID_CDC,wire,layer,ctime,obeta2);
				//std::cout << layer << "-" << wire << " : " << obeta2 << ", " << ctime << std::endl;
			}
			//ctime = 0.0;
...
bool CDSTrack::GetCDHHit2(CDSHitMan *cdsMan, int &seg, double &time, double &diff){
	TVector3 vtxCDH = CDHVertex();
	seg=0;
	time=999;
	diff=999999;
	for(int i=0;i<nCDHHit();i++){
