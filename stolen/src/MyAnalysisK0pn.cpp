...
MyAnalysisK0pn::MyAnalysisK0pn(TFile* rt, ConfMan* conf)
{
  Initialize(conf);
  CutCondition();
  Clear();
...
void MyAnalysisK0pn::Clear()
{
}

bool MyAnalysisK0pn::DoAnalysis(ConfMan* conf, EventHeader* header, BeamLineHitMan* blMan, BeamLineTrackMan* bltrackMan, CDSHitMan* cdsMan, CDSTrackingMan* cdstrackMan, Particle* particle)
{
  ...
  // K0 + Proton
  TLorentzVector Lcds = Lk0 + Lp;
  TLorentzVector cLcds = cLk0 + cLp;
  // Neutron
  ...
  FillHist(Form("k0pn_MMass"),ncds_mmass[1]);
  FillHist(Form("k0pn_MMom"),ncds_mmom[1]);
  FillHist(Form("k0pn_MCosT"),ncds_mcost[1]);

  return true;

}

bool MyAnalysisK0pn::FillHist(TString name, double val1, int weight)
{
...
bool MyAnalysisK0pn::FillHist(TString name, TString val1, int weight)
{
  TH1F* h1 = (TH1F*)gFile -> Get(name);
  if(h1&&weight>0){
    for(int i=0; i<weight; i++){
...
bool MyAnalysisK0pn::FillHist(TString name, double val1, double val2, int weight)
{
...
bool MyAnalysisK0pn::FillHist(TString name, TString val1, TString val2, int weight)
{
  TH2F* h2 = (TH2F*)gFile -> Get(name);
  if(h2&&weight>0){
    for(int i=0; i<weight; i++){
...
void MyAnalysisK0pn::CutCondition()
{
  /* K0 mass cut */ 
  double mean  = 0.4972;
  double sigma = 0.0060;
...
bool MyAnalysisK0pn::Initialize(ConfMan* confMan)
{
  std::cout << "### MyAnalysisK0pn::Initialize ###" << std::endl;

  std::string ofname = confMan->GetOutFileName();
  ofname.insert(ofname.find(".root"),"_anaK0pn");
  ...
  TString pname[4]  = {"k0pn","k0","sblk0","sbuk0"};
  TString pname2[4] = {"K^{0}pn","K^{0}","K^{0}_{lower}","K^{0}_{upper}"};
  for(int ip=0; ip<1; ip++){
    new TH1F( Form("%s_MMass",pname[ip].Data()), Form("Missing Mass (%s);MM(%s) (GeV/c^{2});Counts",pname2[ip].Data(),pname2[ip].Data()), 4000, 0.0, 2.0);
