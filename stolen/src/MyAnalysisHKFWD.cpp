...
#include "MyAnalysisHKFWD.h"

MyAnalysisHKFWD::MyAnalysisHKFWD(TFile* rt, ConfMan* conf)
{
  Initialize(conf);
  CutCondition();
  Clear();
...
MyAnalysisHKFWD::~MyAnalysisHKFWD()
{
  Clear();
  rtFile->cd();
  rtFile->Write();
...
void MyAnalysisHKFWD::Clear()
{
}

bool MyAnalysisHKFWD::DoAnalysis(ConfMan* conf, EventHeader* header, BeamLineHitMan* blMan, BeamLineTrackMan* bltrackMan, CDSHitMan* cdsMan, CDSTrackingMan* cdstrackMan, Particle* particle)
{
  rtFile->cd();
...
bool MyAnalysisHKFWD::DoNeutralAna(Particle* particle, pBeam* beam, pCDS* cds, TVector3 vertex)
{
  DetectorList *dlist=DetectorList::GetInstance();
...
bool MyAnalysisHKFWD::DoChargedAna(Particle* particle, pBeam* beam, pCDS* cds, TVector3 vertex)
{
  DetectorList *dlist=DetectorList::GetInstance();
...
bool MyAnalysisHKFWD::FillHist(TString name, double val1, int weight)
{
...
bool MyAnalysisHKFWD::FillHist(TString name, TString val1, int weight)
{
  TH1F* h1 = (TH1F*)gFile -> Get(name);
  if(h1&&weight>0){
    for(int i=0; i<weight; i++){
...
bool MyAnalysisHKFWD::FillHist(TString name, double val1, double val2, int weight)
{
...
bool MyAnalysisHKFWD::FillHist(TString name, TString val1, TString val2, int weight)
{
  TH2F* h2 = (TH2F*)gFile -> Get(name);
  if(h2&&weight>0){
    for(int i=0; i<weight; i++){
...
void MyAnalysisHKFWD::CutCondition()
{
  /* K0 mass cut */ 
  double mean  = 0.4976;
  double sigma = 0.0069;
...
bool MyAnalysisHKFWD::Initialize(ConfMan* confMan)
{
  std::cout << "### MyAnalysisHKFWD::Initialize ###" << std::endl;

  std::string ofname = confMan->GetOutFileName();
  ofname.insert(ofname.find(".root"),"_anaHKFWD");
