...
bool MyAnalysisBLDCCheck::Initialize(ConfMan* confMan)
{
  ...
  double dtrange[ndc] = {250,250,150,150,100,100};
  double dtmin[ndc]   = {-25,-25,-25,-25,-25,-25};
  double dtmax[ndc]   = {225,225,125,125,125,125};
  ...
  for(int idc=0;idc<ndc;idc++){
    ...
    for( int layer=1; layer<=nlays; layer++ ){
      ...
      for(int wire=1;wire<=nwire;wire++){
        ...
        //new TH1F( Form("%sl%dw%02d_Time_inTrack",name,layer,wire),Form("Time %s-L%dW%02d;Time (ns);Counts",name,layer,wire),dtrange[idc],dtmin[idc],dtmax[idc]);
