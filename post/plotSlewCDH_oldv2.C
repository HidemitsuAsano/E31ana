void plotSlewCDH(const char* filename){
  ...
  conf->Initialize();
  double egatemin = 0.5;
  double egatemax = 49.0;
  TF1 *slewfunc[36*2]; 
  for(int iseg=0;iseg<36;iseg++){
    c1->cd(iseg+1);
    dECDH_T03_CDH_PI[iseg] = (TH2F*) file->Get(Form("dECDH_T03_CDH%d_PI",iseg+1));
    dECDH_T03_CDH_PI[iseg]->Draw("colz");
    ...
    dECDH_T03_CDH_PI[iseg]->Fit("slewfunc","","",egatemin,egatemax);
