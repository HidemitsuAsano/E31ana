...
bool ELossTools::CalcHelixElossPointToPointTGeo(const TVector3 &posin, const TVector3 &momin, const double &mass, const double &z, const double &field, TVector3 &posout, TVector3 &momout, double &flightlength, double &tof){
  // std::cout<<Form("pos in (%5.3lf, %5.3lf, %5.3lf)", posin.X(), posin.Y(), posin.Z())<<std::endl;  
  // std::cout<<Form("mom in (%5.3lf, %5.3lf, %5.3lf)", momin.X(), momin.Y(), momin.Z())<<std::endl;  

  int charge=1.0;
  double param[5];
  if( !MathTools::GetHelix(posin, momin, charge, field, param) ){
    std::cout<<"!!!!! MathTools::GetHelix return false !!!!!"<<std::endl;
    exit(0);
  }
  flightlength=0;
  tof=0;
  ...
  if( mass>0 ){
    posout=MathTools::CalcHelixPosatZ(param, z);
    //    std::cout<<Form("pos out (%5.3lf, %5.3lf, %5.3lf)", posout.X(), posout.Y(), posout.Z())<<std::endl;  
    TVector3 pos1=posin;
    TVector3 mom1=momin;
    TVector3 pos2;
    TVector3 mom2;
    ...
    while( GeomTools::StepToNextVolume(pos1, posout, tmpl, mat2) ){
      double tmp_z=pos1.Z()+(mom1.Z()/mom1.Mag())*tmpl;
      pos2=MathTools::CalcHelixPosatZ(param, tmp_z);
      mom2=MathTools::CalcHelixMom(param, tmp_z);
      if( mom2.Z()<0 ) mom2*=-1.0;
      GeomTools::GetIDMat(pos1,mat1);

      //      tmpl=MathTools::CalcHelixArc(param, pos1, pos2);
      tmpl=(mom1.Mag()/mom1.Z())*(pos2.Z()-pos1.Z());
      double mom_out, tmp_tof;
      if( !ELossTools::CalcdE(mom1.Mag(), mass, tmpl, mat1, mom_out, 1, tmp_tof) ){
	std::cout<<"!!!!! ELossTools::CalcdE("<<mom1.Mag()<<", "<<mass<<", "<<tmpl<<", "<<mat1<<", "<<mom_out<<", 1, "<<tmp_tof<<") !!!!!"<<std::endl;
	exit(0);
      }
      // std::cout<<mat1<<"  "<<mat2<<std::endl;
      // std::cout<<Form("current pos (%lf, %lf, %lf)", pos2.X(), pos2.Y(), pos2.Z())<<std::endl;
      ...
      if( !MathTools::GetHelix(pos1, mom1, charge, field, param) ){
	std::cout<<"!!!!! MathTools::GetHelix return false !!!!!"<<std::endl;
	exit(0);
      }
    }
    pos2=MathTools::CalcHelixPosatZ(param, z);
    mom2=MathTools::CalcHelixMom(param, z);
    if( mom2.Z()<0 ) mom2*=-1.0;
    GeomTools::GetIDMat(pos1,mat1);
    //    tmpl=MathTools::CalcHelixArc(param, pos1, pos2);
    tmpl=(mom1.Mag()/mom1.Z())*(pos2.Z()-pos1.Z());
    ...
    TVector3 dir1=momin.Unit();
    TVector3 dir2=momout.Unit();
    // std::cout<<"momin  dir : "<<dir1.X()<<", "<<dir1.Y()<<", "<<dir1.Z()<<std::endl;
    // std::cout<<"momout dir : "<<dir2.X()<<", "<<dir2.Y()<<", "<<dir2.Z()<<std::endl;

    return true;
  }
  else{
    posout=MathTools::CalcHelixPosatZ(param, z);
    momout=MathTools::CalcHelixMom(param, z);
    if( momout.Z()<0 ) momout*=-1.0;
    flightlength=(momin.Mag()/momin.Z())*(posout.Z()-posin.Z());
    //    flightlength=MathTools::CalcHelixArc(param, posin, posout);
    tof=flightlength/(100.*Const);
    // std::cout<<"CalcHelixElossPointToPointTGet() mass==0"<<std::endl;
    // std::cout<<Form("pos out (%lf, %lf, %lf)", posout.X(), posout.Y(), posout.Z())<<std::endl;
    // std::cout<<Form("mom out (%lf, %lf, %lf)", momout.X(), momout.Y(), momout.Z())<<std::endl;
    // std::cout<<"FL : "<<flightlength<<"  tof : "<<tof<<std::endl;
    return true;
  }

  std::cout<<"!!!!! CalcHelixElossPointToPointTGeo(z="<<z<<") !!!!!"<<std::endl;
  return false;
}
