...
void EventAnalysisMTDC::Initialize( ConfMan *conf )
{
  ...
  if(confMan->GetCDSTrackFileName().c_str()!=DefaultFileName){
    mtdcFile = new TFile( confMan->GetCDSTrackFileName().c_str());
    if(mtdcFile->IsOpen()){
      ...
      std::cout<< confMan->GetCDSTrackFileName().c_str() << " opened"<<std::endl;
...
bool EventAnalysisMTDC::UAna( TKOHitCollection *tko )
{
  ...
  ofs<<header->spillt(0)<<"\t"<<header->evidt(0)<<"\t"<<header->spillt(1)<<"\t"<<header->evidt(1)
     <<"\t"<<spill_gpio<<"\t"<<evid_gpio<<std::endl;
#endif

  if(header->spillt(0)!=header->spillt(1)
     ||header->evidt(0)!=header->evidt(1)
     ||prev_spill!=header->spillt(0)
     ||(prev_evid+1)!=header->evidt(0)
     )
    std::cout<<header->spillt(0)<<"\t"<<header->evidt(0)<<"\t"<<header->spillt(1)<<"\t"<<header->evidt(1)<<"\t<-\t"<<prev_spill<<"\t"<<prev_evid<<std::endl;  
  prev_spill=header->spillt(0);
  prev_evid=header->evidt(0);
#if match
...
// #endif
  ...
  h1=(TH1F*)gFile->Get(Form("h_evidt")); 
  h1->Fill(header->evidt(0));
...
void EventAnalysisMTDC::InitializeHistogram()
{
  ...
  new TH1F(Form("h_evidt"),Form("h_evidt"),5000,-0.5,4999.5);
