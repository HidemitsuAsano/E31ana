...
int main(int argc, char** argv)
{
  ...
  for( int seg=1; seg<=34; seg++ ){
    f-> cd();
    vector<double> adc_param=CalibADC(conf, of, CID_CVC, seg, 6.0);
    xxx[index]=seg;
  ...
  gra_pede_u-> Write("gra_CVC_pede_up");
  gra_pede_d-> Write("gra_CVC_pede_down");
  gra_gain_u-> Write("gra_CVC_gain_up");
  gra_gain_d-> Write("gra_CVC_gain_down");
  gra_thre_u-> Write("gra_CVC_thre_up");
  gra_thre_d-> Write("gra_CVC_thre_down");
