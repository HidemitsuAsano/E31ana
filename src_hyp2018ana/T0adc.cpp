...
int main(int argc, char** argv)
{
  ...
  for( int seg=1; seg<=5; seg++ ){
    f-> cd();
    vector<double> adc_param=CalibADC(conf, of, CID_T0, seg, 2.0);
    xxx[index]=seg;
  ...
  gra_pede_u-> Write("gra_T0_pede_up");
  gra_pede_d-> Write("gra_T0_pede_down");
  gra_gain_u-> Write("gra_T0_gain_up");
  gra_gain_d-> Write("gra_T0_gain_down");
  gra_thre_u-> Write("gra_T0_thre_up");
  gra_thre_d-> Write("gra_T0_thre_down");
