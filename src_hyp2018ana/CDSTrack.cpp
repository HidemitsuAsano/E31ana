...
bool CDSTrack::GetParameters(const int &id, double *aparam, TVector3 &vtx)
{
  ...
  // std::cout<<"GetParameter("<<id<<")"<<std::endl;
  // for(  std::map<int,parContainer>::iterator ite=parCont.begin(); ite!=parCont.end(); ++ite ){
  //   std::cout<<"parCont  key "<<ite->first<<std::endl;
  // }

  //  std::cout<<" retrun false"<<std::endl;
  return false;
}
...
{
  // std::cout<<" Search Hodo hit ";
  // if( cid==CID_CDH ) std::cout<<"CDH"<<std::endl;
  // else if( cid==CID_IH ) std::cout<<"IH"<<std::endl;
  double rin=0,rout=0;
  if(cid==CID_CDH){
...
bool CDSTrack::Retiming(CDSHitMan *cdsMan, ConfMan *conf,double beta,bool SLEW)
{
  ...
  for(int layer=1;layer<=15;layer++){
    for(int n=0;n<(int)nTrackHit(layer);n++){
      ...
      if(SLEW){
	int wire=cdc->wire();
	if( conf->GetSlewingMapManager()->isParam(CID_CDC, layer, wire) ){
	  ctime = conf-> GetSlewingMapManager()-> CalcCValue(CID_CDC, layer, wire, ctime, obeta2);
	}
	else{
  ...
  if(SLEW && fittinglevel<5 ){
    for( int i=0; i<3; i++ ) HelixFitting(cdsMan);
    fittinglevel=5;
  }
