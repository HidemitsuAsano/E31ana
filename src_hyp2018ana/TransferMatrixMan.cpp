...
// + -- + -- + -- + -- + -- + -- + -- + -- + -- + -- + -- + -- + -- + -- + -- + -- + -- + //
TransferMatrixMan::TransferMatrixMan( const std::string & filename )
{
  FileName = filename;
  Clear();
}
...
TransferMatrixMan::TransferMatrixMan( const TransferMatrixMan &right )
{
  ...
  /*
  D5Matrix1st=&right.D5Matrix1st;
  BLC2BLC1Matrix1st=&right.BLC2BLC1Matrix1st;
  for(int i=0;i<6;i++){
   D5Matrix2nd[i]=&(right.D5Matrix2nd[i]);
    BLC2BLC1Matrix2nd[i]=&(right.BLC2BLC1Matrix2nd[i]);
  }
  BLC2VIMatrix=&right.BLC2VIMatrix;
  BLC1VIMatrix=&right.BLC1VIMatrix;
  */
}
// + -- + -- + -- + -- + -- + -- + -- + -- + -- + -- + -- + -- + -- + -- + -- + -- + -- + //
TransferMatrixMan::~TransferMatrixMan()
...
// + -- + -- + -- + -- + -- + -- + -- + -- + -- + -- + -- + -- + -- + -- + -- + -- + -- + //
void TransferMatrixMan::SetFileName( const std::string & filename )
{
  FileName = filename;
}
...
{
  static const std::string funcname = "TransferMatrixMan::Initialize";
...
{
  //par x, x', y, y', mom
  double parin[6]={parblc1[0],parblc1[1],parblc1[2],parblc1[3],0.,
...
{
  //par x, x', y, y', mom
  double parin[6]={parblc1[0],parblc1[1],parblc1[2],parblc1[3],0.,(parblc1[4]-1.)*100.};
  TMatrixD in;
  in.Use(6,1,parin);
  //  in.Print();
  TMatrixD out(6,1);
