...
int main( int argc, char **argv )
{
  ...
  for( int ev=0; ev<tree->GetEntries(); ev++ ){
    ...
    if( T0hits.size()==1 && bltrackMan->ntrackBPC()==1 && bltrackMan->ntrackBLC2()==1 ){
      beam.SetT0(T0hits[0]); beam.SetBLC2id(0); beam.SetBPCid(0); beam.SetPID(Beam_Pion);
      {
	TVector3 pos=bltrackMan->trackBPC(0)->GetPosatZ(-20);
	std::cout<<Form("BPC pos (%lf, %lf, %lf)", pos.X(), pos.Y(), pos.Z())<<std::endl;
      }

      Track *beam_track=MyMCTools::initTrack(mcData, 211); // 321 is PDG ID of K+
      ...
      if( MyMCTools::goodPi_beam(mcData, detData) ){
	beam.SetFlag(true);
	...
	if( beam.nFDC1()==1 && BVChits.size()==1 ){
	  ...
	  if( FChit ){
	    ...
	    info.calc_simple_beam_through(beam, conf, blMan);
