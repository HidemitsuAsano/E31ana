...
bool VEvent::Processing3( ConfMan *confMan )
{
  ...
  if( RecordType==UniRecTypeNormal ){
	...
	if( int(*ph_scan)!=SMP_Header ) std::cout << " !! " << std::endl;
	int num_footer = 0;
	while( int(*ph_scan) != SMP_Footer ){
	  if( (int(*ph_scan)&SD_ModuleDownBit)==SD_ModuleDownBit ) num_footer++;
	  ph_scan++;
	}
#if 0
...
bool VEvent::ProcessingNormal( ConfMan *confMan )
{
  ...
  for( int i=0; i<nbuf; i++ ){
    ...
    while(0<ndat && int(SD_ModuleDownBitMask((*pb))) != SD_ModuleDownBit ){
      int n = SD_GetMA(*(pb));
      int a = SD_GetSA(*(pb));
  ...
  bool status = evTemp->UAna(tkoCol);

  tkoCol->Clear();
  //std::cout << " PN:" << status << std::endl;
  return status;
}
...
bool VEvent::ProcessingBegin( ConfMan *confMan )
{
  ...
  //evTemp = new EventAnalysis();
  evTemp = evAlloc->EventAllocator();
  evTemp->Initialize(confMan);
