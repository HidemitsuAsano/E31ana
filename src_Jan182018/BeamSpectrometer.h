...
class BeamSpectrometer : public TObject
{
  // ---------------------------------------------------- //
 ...
 private:
  ...
  LinearTrack *BLC1Track;
  LinearTrack *BLC2Track;
 ...
 public:
  ...
  LinearTrack *blc1track(){ return BLC1Track;}
  LinearTrack *blc2track(){ return BLC2Track;}
  void SetMomentum(const double &momentum) { Par[4]=momentum; }
  void SetLocalTrack(LinearTrack* blc1,LinearTrack* blc2);
  void TMinuitFit(LinearTrack* blc1,LinearTrack* blc2, ConfMan* conf);
  void Clear();
  void fit(ConfMan* conf);
